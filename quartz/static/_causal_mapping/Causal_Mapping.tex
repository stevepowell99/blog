\documentclass[a4paper,oneside, a4paper]{memoir}
\usepackage{lmodern}
...
\hypersetup{unicode=true,
            pdftitle={Causal Mapping},
            pdfauthor={Steve Powell},
            pdfborder={0 0 0},
            breaklinks=true}
...
\hypertarget{whats-in-this-guide}{%
\chapter{What's in this guide?}\label{whats-in-this-guide}}
...
There will also be general materials on harder issues in the coding and meaning of causal networks\ldots{}
...
\hypertarget{manifesto}{%
\chapter{Manifesto: piecing together fragments of causal information}\label{manifesto}}

Gone are the days when we could think of data or information as primarily about \emph{numbers}. Many of us who are involved in understanding the social world and evaluating interventions within it spend much of our day understanding, presenting and manipulating \emph{causal structures} and even \emph{models of other people's} causal structures.
...
\begin{quote}
A \textbf{causal map} is a model of causal relationships, in the form of a directed graph in which the items (nodes, elements ) are linked by arrows, together with translation rules which tell us how to interpret the arrows, namely that an item with one or more arrows pointing to it is in some sense \emph{causally influenced by} the item(s) at the start of those arrow(s).\footnote{In most cases the model is literally drawn in the form of a network, but other ways of representing the same ideas are possible too, for example a set of narrative claims.}
\end{quote}
...
\begin{quote}
A (causal) \textbf{mini-map} is one item together with all the items directly pointing to it, i.e.~directly influencing it.
\end{quote}

The mini-map rule helps us translate ``drought --\textgreater{} hunger'' to the equivalent causal claim (``drought causally influences hunger''), and back again. Larger causal maps are built up from bunches of mini-maps.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-2-1.pdf}
Fig 1. A mini-map

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-3-1.pdf}
Fig 2. Another mini-map

Different kinds of causal maps can have radically different translation rules: what precise flavour of causation is meant? The causal arrow might mean ``\ldots{}completely determines\ldots{}'' or ``\ldots{}is sufficient for\ldots{}''. Or arrows might have numbers associated with them, and the arrow might be interpreted as something like ``\ldots{}increases by a factor of .5 the probability of \ldots{}''. In most causal maps which programme evaluators have to deal with, the meaning of these arrows can be mixed and unclear, often meaning just ``\ldots{} causally influences in some way \ldots{}''

The \emph{elements} within different kinds of causal map (like ``drought'') differ correspondingly. In some of the cases above, the elements might be \emph{propositions} like ``drought happened''; in others, they are continuous variables; other kinds are possible too.

But a causal map's translation rules have to be explicitly causal. As Pearl points out, a genuinely causal arrow can be understood as something like ``if you \emph{do} X, Y will happen (perhaps, with probability \(p\))'', whereas, for example, Bayesian belief networks are not causal maps because the arrows say ``if you \emph{observe} X, you will observe Y (perhaps with the probability \(p\))''. Causation is not correlation.

To make them useful, causal maps nearly always come with additional \emph{combination} rules which tell us how we can piece together a larger map out of smaller ones, and how to understand it on the basis of our understanding of its constituent parts. We can use these rules for example to make deductions about maps or to derive a simpler version of a larger map. As both Realist theorists and the Pearlian school point out, our real-life knowledge is composed of relatively portable, transferrable small units like mini-maps.

For example, if we know that

\begin{quote}
drought --\textgreater{} hunger
\end{quote}

and

\begin{quote}
hunger --\textgreater{} revolution
\end{quote}

can we deduce

\begin{quote}
drought --\textgreater{} revolution?
\end{quote}

If we are told to interpret ``--\textgreater{}'' as ``completely determines'', this is arguably an acceptable conclusion; likewise with ``may have some causal influence on''. But if we interpret it as ``increases the probability of X by 50\%'', this conclusion doesn't work, because we are trying to describe a partial influence which we expect to \emph{attenuate} as we go down the causal chain. Concretely, we'd then expect the influence of drought on hunger to be less than 50\%.

So, the explicit or implicit rules which come with a causal map also help us to make \emph{deductions} with maps.

\hypertarget{causal-map-paradigms}{%
\section{\texorpdfstring{Causal map \emph{paradigms}}{Causal map paradigms}}\label{causal-map-paradigms}}

We don't have to set up these kinds of rule for understanding causal arrows every time we draw a causal map. Instead, we make use of an established \emph{paradigm}: we only need to say ``this is a structural equation model'' or ``this is a fuzzy cognitive map'', and the rules are understood. A causal map paradigm is just a frequently-used set of rules for understanding and manipulating a whole genre of causal maps.
...
\begin{quote}
The horse kicked me --\textgreater{} I screamed
\end{quote}
...
\hypertarget{mini-maps-with-more-than-one-influencing-item}{%
\section{Mini-maps with more than one influencing item}\label{mini-maps-with-more-than-one-influencing-item}}

Many causal map paradigms assume that causal influences are always between a single cause and a single effect. This is a very useful simplification. But sometimes we need to take into account causal connections in which something is affected by sets of more than one influence. For example, what if we want to draw a causal map to show that you need both oxygen \emph{and} a spark to cause a fire? This can't be assembled from two separate diagrams:
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-4-1.pdf}

This can't be expressed like this:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-5-1.pdf}

\hypertarget{semi-formal-definition-of-a-causal-map-paradigm}{%
\section{Semi-formal definition of a causal map paradigm}\label{semi-formal-definition-of-a-causal-map-paradigm}}

A causal map paradigm is a set of rules for constructing and understanding causal maps. It should contain as a minimum:

A \textbf{mini-map rule} defining what counts as a ``mini-map'' within this paradigm (roughly, a ``consequence'' element with one or more ``influence'' elements pointing to it); it may have other attributes too like factual information about the actual state of things, or the strength of the influences, the amount of trust in the information, etc. The rule has two parts saying (syntax): any such mini-map counts as a causal map and (semantics): how to translate such a mini-map into a particular kind of causal claim, and back.

\textbf{Combination rules} showing how to build up larger maps. The rules each have two parts saying (syntax): mini-maps joined in such-and-such a way also count as causal maps and (semantics): what does the composite map mean in terms of the meanings of its elements?
...
\hypertarget{hybrid-sets-of-rules}{%
\section{Hybrid sets of rules}\label{hybrid-sets-of-rules}}
...
\hypertarget{causal-multi-maps}{%
\section{\texorpdfstring{Causal \emph{multi-maps}?}{Causal multi-maps?}}\label{causal-multi-maps}}

In the approaches listed above, the information underlying them can usually be considered to come from just one source: one book, or one experiment, or one survey, or one expert; or the question ``what is the source for that'' isn't even raised at all. We can call these kinds of maps ``causal \emph{mono-maps}''. They are particularly interesting for us at Bath SDR because they are at the heart of the QuIP methodology. Our causal map app is specialised in dealing with them.

A causal \emph{multi-map} is a kind of causal map in which information about the source of each causal link (or more generally of each mini-map with several influencing items) is firmly attached to it and taken very seriously. The rules for this kind of map need to take this information into account, as in the following example.
...
..\ldots{} and we can't even necessarily deduce this:
...
The term ``\textbf{causal mapping}'' has often been used to acknowledge the possibility that causal information may comes from different sources and explicitly opens the challenge of how to meaningfully aggregate that information.

\hypertarget{examplesOfMaps}{%
\section{A brief overview of different causal map paradigms}\label{examplesOfMaps}}

\hypertarget{approaches-already-called-causal-mapping}{%
\subsection{Approaches already called ``Causal Mapping''}\label{approaches-already-called-causal-mapping}}
...
\hypertarget{comparative-cognitive-mapping}{%
\subsection{(Comparative) Cognitive Mapping}\label{comparative-cognitive-mapping}}

\hypertarget{theories-of-change-for-a-project-or-programme}{%
\subsection{Theories of Change for a project or programme,}\label{theories-of-change-for-a-project-or-programme}}
...
\hypertarget{programme-theories-in-theory-based-evaluation}{%
\subsection{Programme theories in theory-based evaluation}\label{programme-theories-in-theory-based-evaluation}}

\hypertarget{fuzzy-cognitive-maps}{%
\subsection{Fuzzy Cognitive Maps}\label{fuzzy-cognitive-maps}}

\hypertarget{contribution-analysis}{%
\subsection{Contribution analysis}\label{contribution-analysis}}

\hypertarget{systems-diagrams}{%
\subsection{Systems Diagrams}\label{systems-diagrams}}

\hypertarget{dags-as-promoted-by-judea-pearl}{%
\subsection{DAGs as promoted by Judea Pearl}\label{dags-as-promoted-by-judea-pearl}}
...
\subsection{Structural Equation Models}\label{structural-equation-models}}

\hypertarget{bayesian-belief-networks}{%
\subsection{Bayesian belief networks}\label{bayesian-belief-networks}}

probably not, see Pearl (1986)

\hypertarget{arguably-diagrams-used-in-realist-evaluation}{%
\subsection{(arguably) diagrams used in Realist Evaluation}\label{arguably-diagrams-used-in-realist-evaluation}}

\hypertarget{sometimes-diagrams-used-in-outcome-harvesting}{%
\subsection{(sometimes) diagrams used in Outcome Harvesting}\label{sometimes-diagrams-used-in-outcome-harvesting}}

\hypertarget{causal-maps-as-constructed-in-quip}{%
\subsection{Causal Maps as constructed in QuIP}\label{causal-maps-as-constructed-in-quip}}

\hypertarget{mental-models}{%
\subsection{Mental models}\label{mental-models}}

Moon et al. (2019) from the field of conservation, with a lot of emphasis on complexity and systems. Also ask about what respondents say \emph{should} be the case. Use 1, 2, or 3 for strength.
...
\hypertarget{knowledge-graphs-semantic-networks}{%
\subsection{Knowledge graphs, semantic networks}\label{knowledge-graphs-semantic-networks}}
...
\hypertarget{nca-necessary-condition-analysis}{%
\subsection{NCA (Necessary Condition Analysis)}\label{nca-necessary-condition-analysis}}
...
\hypertarget{paradigms-which-include-the-aspect-of-belief}{%
\subsection{Paradigms which include the aspect of belief}\label{paradigms-which-include-the-aspect-of-belief}}

Modal logic of belief, Dempster-Shafer theory

\hypertarget{system-effects}{%
\subsection{``System Effects''}\label{system-effects}}

\url{https://www.lukecraven.com/system-effects/}

\hypertarget{quip-maps}{%
\subsection{QuIP maps}\label{quip-maps}}

The raw output of a QuIP-type coding exercise is a set of directed paths or arrows between a set of nodes or factors, in which the arrows might have additional attributes like respondent characteristics, question number / domain etc. One or more arrows B, maybe also C, D, ..\ldots{} coming into a node E encode a causal claim about how B (and maybe C and D..\ldots{}) influence E.

\hypertarget{maps-of-maps}{%
\subsection{Maps of maps}\label{maps-of-maps}}
...
The \textbf{causal map app} is a tool which helps social scientists and programme evaluators to extract causal claims from texts (e.g.~interview data) from different sources, and to combine and present it visually.

\hypertarget{causal-mapping}{%
\chapter{Causal mapping}\label{causal-mapping}}
...
Information about causes, or at least about causal contributions, or at least about people's informed opinions about causal contributions; all of this is gold to a programme evaluator or social scientist. We want and need to be able to collect, store, process, combine, analyse and display this kind of information, and do it easily and well. In the evaluation literature there is less discussion of these kinds of task compared to the amount of discussion around the questions of how one should collect, store, process, combine, analyse and display \emph{numerical} or even so-called \emph{qualitative} information as usually understood.
...
Causal maps are obviously good for the tasks of \emph{displaying} sets of causal claims, but we aren't used to thinking of diagrams or networks also as a way of \emph{storing} information. But they are; and there is a whole range of modern software tools such as graph databases which do just that, in which the basic unit of information is not a number or a sentence but a link between two or more items.

Causal maps are also useful as \emph{summaries} of causal information, central to much research and evaluation: a way of expressing the main findings, what we really wanted to know: does X causally influence Y? Did B contribute to C? What else has to happen? How sure can we be?
...
\begin{figure}
\centering
\includegraphics{assets/clip_image002.png}
\caption{1573059919382}
\end{figure}

Next, you view each statement one by one, highlighting passages which contain causal claims. For each claim, you decide what is the cause and what is the effect.\includegraphics{assets/clip_image004.png}

Each causal claim becomes part of a small, ``live'' causal map in the right-hand window which grows as you work.

\begin{figure}
\centering
\includegraphics{assets/clip_image006.png}
\caption{1573060610020}
\end{figure}

Last, you can view the whole map built up from all the causal claims you have been reviewing. You can search, change, edit and simplify the codes you have been using, as well as getting an overall picture: what caused what in these stories? You can also conduct more sophisticated analyses and produce graphics and reports, answering questions like: \emph{did the stories which the women told differ from those which the men told}?

\begin{figure}
\centering
\includegraphics{assets/clip_image008.png}
\caption{1573061020009}
\end{figure}

\hypertarget{soft}{%
\chapter{We need a ``soft arithmetic'' for causal maps}\label{soft}}

\includegraphics{assets/swirl.png}

The forthcoming sections are the main theoretical contribution of this guide. The are an attempt to put together a kind of ``soft arithmetic'' for causal maps: a set of rules which tell us how to combine different pieces of causal information and how to make deductions with them. To do so, we have to address a wide and challenging range of issues. Many of these issues are familiar to practitioners like QuIP coders, indeed to anyone who tries to piece together fragments of causal information, but in most cases there is no consensus on how they are to be answered. I'll be arguing that \textbf{if we want to try to address even the most basic needs of evaluation users, we are compelled to try to make some use of some sort of numbers when encoding causal information} -- but in such a way as to keep things as fuzzy as they really are, rather than trying to demand or claim an unrealistic level of precision.

\hypertarget{an-ambitious-project}{%
\section{An ambitious project}\label{an-ambitious-project}}

Yes, this attempt to find a common logic behind the different approaches -- from Theories of Change to Structural Equation Models -- that come under the rubric of causal maps -- see the list \protect\hyperlink{overview}{here} -- is ambitious.

\hypertarget{users-of-a-causal-map-expect-to-be-able-to-deduce-some-kind-of-comparative-information-from-it}{%
\section{Users of a causal map expect to be able to deduce some kind of comparative information from it}\label{users-of-a-causal-map-expect-to-be-able-to-deduce-some-kind-of-comparative-information-from-it}}
...
\ldots{} as well as, of course, questions about combinations, like:
...
\ldots{} again, we can have caveats and discussions about who decides, who participates etc; and we should tirelessly emphasise the high level of fuzziness and uncertainty involved. Often we will answer questions like those above with ``sorry but we cant really be sure''. But still, if we \emph{always} answer ``there were a lot of links between C and E, but we have no further information'', we would not be doing our job. It would be like a telecoms agency delivering only metadata about the communications history of a criminal -- certainly not useless, but a thousandth as useful as knowing what was actually said, whether in detail or in summary. Also we would not be doing the main job of an evaluator, according to Michael Scriven Scriven (2012) which is not just to describe what happened but to (help) judge is that \emph{good enough}.

By all means we can let our evaluation stakeholders play with the encoded raw data and let them ``draw their own conclusions''. But in order to draw their conclusions they have to know at least implicitly \emph{how to draw conclusions from causal maps}. They might have better information than we do about the \emph{content}, but our job as experts in causal maps is to advise them specifically on \emph{how to draw conclusions}, how to make decisions about which parts to focus on, how to summarise them, and which parts to filter out.

\hypertarget{asking-and-answering-those-kinds-of-typical-questions-of-a-causal-map-boils-down-to-assigning-some-kinds-of-numbers-to-its-elements}{%
\section{Asking and answering those kinds of ``typical questions'' of a causal map boils down to assigning some kinds of numbers to its elements}\label{asking-and-answering-those-kinds-of-typical-questions-of-a-causal-map-boils-down-to-assigning-some-kinds-of-numbers-to-its-elements}}

The point is that these statements all presuppose, make use of, a kind of ``soft arithmetic'' for causal maps. To be able to make any kind of ``bigger than/smaller than'' claims about causal maps, we need the individual arrows to have, as a minimum, a (probably hidden) property like, in some sense, ``strength'' and ``direction'' as well as a way of comparing those properties. That is, \emph{we are already doing} a kind of vague arithmetic. To be able to make double comparisons like ``The influence of B on E is bigger than the influence of C on E'' we are essentially implying that the arrows have some kind of numbers attached to them. That is in a sense the definition of what numbers \emph{are}: attributes which justify comparisons. It remains to be seen what kinds of numbers those would be.
...
\emph{I} don't want to put numbers on the maps either. But numbers in some form have to be there in the background, however flexible and fuzzy and inaccurate, or we couldn't make useful comparisons.
...
\hypertarget{arent-there-strategies-to-encode-causal-information-without-using-any-kind-of-number}{%
\section{Aren't there strategies to encode causal information without using any kind of number?}\label{arent-there-strategies-to-encode-causal-information-without-using-any-kind-of-number}}
...
By all means, there might be some such strategies, which have various severe limitations. I've addressed the issue \protect\hyperlink{universal}{here}.

\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}

\textbf{A technical note}
...
But we have some advantages over the statisticians too. For one thing, the special status of \emph{causal maps} within quantitative social science, and the fact that they are a completely different kind of creature from mere correlative structures, Pearl (2000), is still controversial in statistics.
...
\hypertarget{meaning}{%
\chapter{We need rules about how to encode causal information in a causal map (and decode it again)}\label{meaning}}
...
For the moment, we are not at all interested in which particular medium --- for example, sentences as opposed to diagrams --- we want to use to encode causal information, because we want to understand the rules which should govern the use of the symbols in \emph{any} such encoding. (Spoiler: in fact we will be using diagrams containing arrows between boxes, basically, for this task.)
...
\hypertarget{how-do-causal-claims-work-within-ordinary-language}{%
\section{How do causal claims work within ordinary language?}\label{how-do-causal-claims-work-within-ordinary-language}}
...
\hypertarget{understanding-the-elements-of-a-causal-map-by-agreeing-how-to-make-deductions-with-them}{%
\section{Understanding the elements of a causal map by agreeing how to make deductions with them}\label{understanding-the-elements-of-a-causal-map-by-agreeing-how-to-make-deductions-with-them}}

Causal arrows are not conduits for a mysterious force caused ``causation''. What flows down the arrows, so to speak, are actual forces - magnetism, peer pressure, greed, whatever. The arrows are the conduits for these \emph{actual} forces: the form, not the content.
...
Instead we will learn its ``meaning'' in precisely the way we learn the ``meaning'' of arithmetical symbols like \texttt{+} for addition: by learning the rules by which we may draw inferences between sentences containing such symbols.\footnote{If we were doing formal logic, we could say that we want to establish the \emph{axioms} and \emph{theorems} for sentences (or diagrams) involving the causal arrow.}

I claim that this problem about \emph{meaning} can be solved by agreeing on how to \emph{reason} with causal maps, to do causal inference. The causal map app needs to have the right rules for causal inference in order to be able to bring the maps alive --- so that we can ask them questions like ``does variable B have more influence on E than variable C?'' or give them instructions like ``Hide all the variables which only have a small influence on E.''

When we learn the meaning of \texttt{+} in school, words don't help us much, we learn by making deductions; we learn that from
...
As far as possible, we will try also to identify symbols and conventions which \emph{fuzzily} capture the fuzziness, incompleteness, ambiguity etc of the way people talk about causal relationships and of the supposed causal relationships themselves.

\hypertarget{meaning-is-arithmetic}{%
\chapter{Soft arithmetic is also the answer to understanding causal maps}\label{meaning-is-arithmetic}}

In section \ref{soft} I claimed that in order for our causal maps to be of any use to anyone, we need to, at least sometimes, attempt to make comparisons between different elements, and comparisons of comparisons, like ``B is more important for E than C is''. This amounts to being able to assign some kind of numerical quantities to elements of the maps, so that we can make deductions with our maps following \textbf{rules which I call ``soft arithmetic''}. In lots of occasions we won't be sure enough to be able to make any such statements, but in others the evidence is so strong that we wouldn't be doing our job if we didn't.

Then in section \ref{meaning} I claimed that we need rules for understanding causal maps: how do we turn causal information from stakeholders into features in a diagram, and in reverse, how do we read a diagram to decode the causal information. I argued that the best way to agree or understand these \textbf{translation rules for causal maps} is to show how to make deductions with them. Just as we don't waste time in primary school explaining what ``\texttt{+}'' means; instead we show how to \emph{use} it.

So it won't be a surprise if I now point out the obvious: the two claims are one and the same, and the two sets of rules are one and the same. \textbf{The rules we will look at in the following sections will tell us both how to do ``soft calculations'' with causal maps and tell us what the elements mean, so that we can encode causal information into maps and back again.}

\hypertarget{metalanguage-some-words-for-talking-about-causal-maps}{%
\chapter{Metalanguage: some words for talking about causal maps}\label{metalanguage-some-words-for-talking-about-causal-maps}}

We will need some metalanguage to talk about causal maps. Metalanguage is not part of the maps, it is a part of English which we use to talk \emph{about} the maps. I've spent quite a lot of time trying to get the terminology right, I'm not making up words for the sake of it.

\hypertarget{the-cause-a-cause-causal-influence}{%
\section{``The cause'', ``a cause'', ``(causal) influence''}\label{the-cause-a-cause-causal-influence}}
...
For similar reasons, in the diagram below I prefer to say that C \emph{influences} or \emph{causally influences} E. Indeed we can and should also say that the \emph{package} of B and C together causally influences E. This makes it clear that we are talking about causation, not for example correlation, but that we are not of course claiming that E is completely determined by C or B.

\hypertarget{nodes-variables-vertices}{%
\section{Nodes, variables, vertices \ldots{}}\label{nodes-variables-vertices}}

From a network perspective, the boxes can be referred to as nodes or vertexes. I will refer to these mostly as \texttt{variables}. However, calling them ``variables'' means that they are things which can vary, be different, for example the temperature outside now, or the extent of last season's rains; sometimes, variables don't seem like quite the right fit when coding causal information which talks about \emph{events}. See \protect\hyperlink{propositions}{later}.

The lines are referred to as (directed) edges, paths or links. I will call them \texttt{arrows}.

\hypertarget{influence-variable-consequence-variable-package-of-variables}{%
\section{Influence variable, consequence variable, package of variables}\label{influence-variable-consequence-variable-package-of-variables}}

In a map like this, we call the variables on the left the \emph{influence variables} of E, and E is the \emph{consequence variable} of B and C. We can also say that B is a \emph{parent} of E and E is a \emph{child} of B. And we can say that both E and F are \emph{downstream} of B and C, and that they are \emph{descendants} of B and C, and so on. We can also say that B, C etc are members of a causal \emph{package} -- provided they are indeed coded together as such, see xx.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-6-1.pdf}

\hypertarget{mechanism-theory-.}{%
\section{Mechanism, theory \ldots{}.}\label{mechanism-theory-.}}
...
\hypertarget{part-the-rules-of-soft-arithmetic-for-causal-maps}{%
\part{The rules of Soft Arithmetic for causal maps}\label{part-the-rules-of-soft-arithmetic-for-causal-maps}}

\hypertarget{mini-map-rule}{%
\chapter{The mini-map coding rule}\label{mini-map-rule}}

Here is the first rule of Soft Arithmetic for causal maps.

\begin{quote}
Information like ``the influence variables B, C and D all have some kind of causal influence on the consequence variable E'' can be coded with a \texttt{mini-map} in which one or more variables (the ``influence variables'') are shown with arrows leading to another (``the consequence variable''). The information and the map are equivalent.
\end{quote}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-7-1.pdf}

Fig xx. A ``mini-map'': one or more variables are shown with arrows leading to one other variable.

In this extreme case, there is no information at all about the \emph{content} of the causal statement from something as vague as ``B and C and D all somehow contribute positively to E'' to more detailed claims. Later we will see different ways in which we might encode that content. But we don't in principle need any specific way to do that, we can just write what we know on the downstream box:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-8-1.pdf}

Fig xx: A mini-map in which the information about the \emph{content} of the causal influence has been written on E, preceded by three dots \texttt{...} . The three dots indicate that what comes after is not formally part of our Soft Arithmetic.
...
It is really important to understand that a mini-map can encode \emph{any} combined influence of a causal package of variables on another, not just the kind of cases we expect when we come from a background of linear, additative models. From that background, we might have to remind ourselves ``aha, there might be interactions between the members of the causal package'' but if you say this you are only showing that you have not yet shaken off the shackles of linear thinking, because the \emph{usual} case is that when B and C are part of a causal packet, the influence of B on E depends on the value of C and vice versa. Independence is a \emph{special case}.

\hypertarget{meaning-depends-also-on-elicitation-context}{%
\section{Meaning depends also on elicitation context}\label{meaning-depends-also-on-elicitation-context}}
...
It's crucial that a mini-map codes information about causality, not co-incidence. So the causal map ``C → E'' should \textbf{not} be interpreted along the pattern of ``if you observe (a high level of) C you are more likely to observe (a high level of) E'', though that may or may not be a corollary of the causal information. The strongest and most correct interpretation is ``if you intervene in the system and manipulate C, which may involve breaking any causal links from other factors to C itself, then this manipulation will produce a corresponding effect in E''.

\hypertarget{rules-mini-func}{%
\chapter{The mini-map coding rule -- functional form}\label{rules-mini-func}}

The \protect\hyperlink{mini-map-rule}{mini-map coding rule} said that Information like ``the influence variables B, C and D all have some kind of causal influence on the consequence variable E'' can be coded with a mini-map in which one or more variables are shown with arrows leading to another.
...
More generally, the function will in real-life cases not completely determine the value of E. Other, mostly unknown, factors are involved. With classical, linear functions we can just \textbf{add} the influence of \(f\) to the prior value of E. (Or conversely we can express the value of E as being a main prediction made by the function \(f\) together with additional ``error'' influences which can likewise be \textbf{added} to the prediction.) As my approach will be broader and will cover non-parametric and non-numerical cases, we cannot rely on this convenient convention. So we should more generally express the function like this: \[E_{posterior} = f(B, C, D, E_{prior})\]. What this says is that \(f\) shifts the value of E away from our previous or prior best guess about E to a value determined not only by the influence variables but also that prior value.
\end{quote}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-9-1.pdf}

\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}

\textbf{Technical note}

It is important to note that the \texttt{=} sign in these functional expressions is not the usual equality sign. For one thing, it is not symmetric. See Pearl (xx).

\hypertarget{the-extras-rule-adding-extra-information-in-particular-about-the-levels-or-values-of-the-variables}{%
\chapter{The extras rule: adding extra information, in particular about the levels or values of the variables}\label{the-extras-rule-adding-extra-information-in-particular-about-the-levels-or-values-of-the-variables}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-10-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-11-1.pdf}

In fact, we already did this when we added a plain-English description of the function to the label of the consequence variable in the \protect\hyperlink{rules-mini-func}{mini-map coding rule}

The three dots \texttt{...} indicate that what comes after is not formally part of our Soft Arithmetic.
...
It just says, you can move information about the levels of the variables formulated in English sentences into similar statements on the variables themselves within the map, and back again. The extra information is \emph{not part of soft arithmetic} in the narrower sense. We don't (yet) have soft arithmetic rules for manipulating it, but we can use ordinary common sense.

\hypertarget{corollary-ordinary-reasoning}{%
\section{Corollary: Ordinary reasoning}\label{corollary-ordinary-reasoning}}

From the rules above it follows that you do not need an extra rule to do ordinary reasoning with causal maps. So from the above map you can also deduce the map below. (To check, remember that we could use the Extras Rule to covert the map into the map without the extra information plus the English sentence ``B = 22'' and from this we can use ordinary reasoning to get the English sentence ``B \textless{} 30'' which we can then combine with the map without the extra information to give the version below.)

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-12-1.pdf}

\hypertarget{the-juxtaposition-rule}{%
\chapter{The juxtaposition rule}\label{the-juxtaposition-rule}}
...
\hypertarget{rules-for-joining-maps}{%
\chapter{Rules for joining maps}\label{rules-for-joining-maps}}
...
This section introduces the four cases from a purely \emph{structural} perspective. The rules in this section will probably seem trivial. They basically all say ``yes, of course you can join together maps on their common variables''.

In the subsequent sections we will look at the more tricky question of how the functions which contain the \emph{causal content} of the mini-maps can be combined.

\hypertarget{chaining-rule}{%
\section{The chaining rule}\label{chaining-rule}}

\begin{quote}
Given two causal maps with one common variable which is an \textbf{influence variable in one map and a consequence variable in the other}, you can derive the single map which chains them together, \emph{providing this does not create a loop}.
\end{quote}

So from this:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-16-1.pdf}

you can deduce this (and vice-versa):

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-17-1.pdf}
...
\hypertarget{shared-consequence-rule}{%
\section{The shared consequence rule}\label{shared-consequence-rule}}

\begin{quote}
Given two causal maps with one common variable which is a \textbf{consequence variable} in both maps, you can derive the single map which combines them.
\end{quote}

From this \ldots{}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-18-1.pdf}

\ldots{} we can derive this:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-19-1.pdf}

\hypertarget{shared-influence-rule}{%
\section{The shared influence rule}\label{shared-influence-rule}}

\begin{quote}
Given two causal maps with one common variable which is an \textbf{influence variable} in both maps, you can derive the single map which combines them.
\end{quote}

From this:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-20-1.pdf}

derive this:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-21-1.pdf}

\hypertarget{merge-same-arrow}{%
\section{The shared arrow rule}\label{merge-same-arrow}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-22-1.pdf}

..\ldots{} we can combine the mini-maps, like this:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-23-1.pdf}
...
\hypertarget{rules-for-joining-maps-what-counts-as-the-same-variable}{%
\chapter{Rules for joining maps: what counts as ``the same'' variable?}\label{rules-for-joining-maps-what-counts-as-the-same-variable}}

We already saw that we will very often have different fragments of information which share common variables, which we can join up to create more comprehensive maps. In the previous section we looked at four different ways to snap together causal maps. They basically all say ``yes, of course you can join together maps on their common variables''. However this does open up the question, critical for people doing actual coding, of \emph{what counts as a common variable}.

We also need to deal with different possible formulations of variable names using what you could call different \emph{parts of speech} like ``more investment'' / ``amount of investment'' / ``investment increased 10\%'' / ``investment will increase 10\%''.

{[}Need to add more here!{]}

\hypertarget{zooming-out}{%
\chapter{The chaining rule, functional form (zooming out / black-box rule)}\label{zooming-out}}

\begin{quote}
Given two mini-maps containing the paths \(E = f(B)\) and \(O = g(E)\) respectively, the \textbf{direct influence} of B on O is \(O = g(f(B))\).
\end{quote}

This rule tells us how to calculate downstream effects in a map which has been created using the \protect\hyperlink{chaining-rule}{chaining rule}.

It is an elementary application of the ``substitution rule'' from secondary school maths.

This problem is familiar from statistics. We can ask, if B affects E and E affects O, what is the \textbf{direct effect} of B on O? If we know \(f\) and \(g\), we can trivially calculate \(g(f(B))\) simply by applying \(g\) to the result of \(f\).
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-24-1.pdf}

..\ldots{} you can in some sense ``zoom out'', create a black box, subsuming the functional content of the pink boxes, to conclude this:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-25-1.pdf}

\ldots{} where the function \(g\) can in principle be constructed if you know \(f1\), \(f2\) and \(f3\).

\hypertarget{two-special-cases}{%
\section{Two special cases}\label{two-special-cases}}
...
\hypertarget{bare-influence}{%
\subsection{Bare influence}\label{bare-influence}}

Consider the very simple function \(H\) such that \(E = H(B)\) which just means, B has \emph{some} influence, a bare influence, on E. Then from
...
\hypertarget{total-control}{%
\subsection{Total control}\label{total-control}}

Whereas in the second extreme version, the interpretation of the function \(H\) is ``the influence variables all have \emph{total causal control} of the consequence variable''. There is no room for E to do anything not dictated by B (and possibly, simultaneously by C, D etc).
...
\hypertarget{shared-consequence}{%
\chapter{The shared consequence rule, functional version}\label{shared-consequence}}
...
But also, according to the equally innocuous-seeming ``mini-map coding rule'', the new map says that there is a \emph{single} rule which (at least partially) determines E given information about B and C. Suddenly this is a big deal. What is that rule? How can we construct the correct function from \(f\) and \(g\)?
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-26-1.pdf}

What is the new causal function implied by this new mini-map?

..\ldots{} we have two single-variable packages which influence a variable ``hunger'' or ``amount of hunger'', the first perhaps expressing a necessary condition, the second perhaps expressing a continuous relationship. How can we combine this information to give us a single function of the form
...
In this section I suggest that there is no perfect solution to calculating \(h\). However I do \protect\hyperlink{add-influences}{later} make a suggestion.

We could ask whether the constituent maps are considered to complement or contradict one another; and I recommend understanding them as contradictory by default. That means that if someone says ``drought drives hunger'' we assume that they \emph{are} making an exclusive claim, and \emph{would} object to the simultaneous claim ``poverty drives hunger''; although if there is both drought and poverty, we would probably expect the level of hunger to be higher than if there was only drought or only poverty.
...
One alternative would be to assume that only when the maps which we are merging come from the same source\footnote{We will discuss the concept of a source in section xx} may we assume that the information is complementary. When they come from different sources, it is less clear how we should combine the information but perhaps we would prefer to default to treating them as contradictory.
...
And yet, we can hardly insist that for every random two pieces of causal information there has to be another which tells us how to combine them. If a farmer tells us ``drought leads to hunger'' we can't go back to them and say, aha, but how does this rule apply in the presence of information about how poverty leads to hunger? Or, worse, in the presence of \emph{each and every} specific piece of information about how anything leads to anything else?
...
\hypertarget{contradictory-combinations}{%
\section{Contradictory combinations}\label{contradictory-combinations}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-27-1.pdf}

\ldots{} but this might feel weird. We would perhaps prefer to work out which putative cause was actually effective. Or we might suggest \href{https://en.wikipedia.org/wiki/Exclusive_or}{XOR} as the relevant function.
...
\hypertarget{but-which-case-is-which}{%
\section{But which case is which?}\label{but-which-case-is-which}}

In a later \protect\hyperlink{add-influences}{section} I present a default rule for combining complementary causal maps. And above I started to sketch one for combining contradictory causal maps. The big problem is that we don't necessarily know which one to use. This has to do with whether we think of ``G -\textgreater{} E'' as meaning
...
\ldots{} and on the other hand, in this case
...
\begin{quote}
When maps are combined using the ``same consequence'' rule, we will \textbf{assume the constituent maps are contradictory rather than complementary} unless we are told otherwise.
\end{quote}
...
\hypertarget{extension-to-causal-packages}{%
\section{\texorpdfstring{Extension to causal \emph{packages}}{Extension to causal packages}}\label{extension-to-causal-packages}}

Above we have only considered mini-maps with single influence variables. In general, mini-maps show how several influence variables together influence the consequence variable as \emph{a causal package}. The logic is the same, but if we combine two causal packages, we have to remember the packages and which variable belongs to which package. We can't just smash them all together (unless we make very strong linearity assumptions). This means that in this example, there are two causal packages, one which combine to influence E via logical AND, the other via logical OR (but the same applies to any kind of functional package).

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-28-1.pdf}

Here the variables have been coloured to visually record which variable belongs to which package (and in general, there is nothing to stop the same variable belonging to \emph{both} packages). Our app has to remember this too (and indeed it does).

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-29-1.pdf}

\hypertarget{shared-influence-function}{%
\chapter{The shared influence rule, functional version}\label{shared-influence-function}}

What about when we combine maps according to the shared influence rule?

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-30-1.pdf}
...
\hypertarget{the-shared-arrow-rule-functional-form.}{%
\chapter{The shared arrow rule, functional form.}\label{the-shared-arrow-rule-functional-form.}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-31-1.pdf}
..In the absence of any other information, we perhaps suggest some kind of average influence.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-32-1.pdf}
...
\ldots{} we are usually only too happy to combine the mini-maps. The combined function can be calculated as in the previous section.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-33-1.pdf}

\hypertarget{the-rule-for-merging-arrows.-weight-of-evidence.}{%
\chapter{The rule for merging arrows. Weight of evidence.}\label{the-rule-for-merging-arrows.-weight-of-evidence.}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-34-1.pdf}
\ldots{} and have already combined this into a single map:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-35-1.pdf}
\ldots{} we can show this as a single arrow:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-36-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-37-1.pdf}

\hypertarget{problems-with-evidence}{%
\section{Problems with evidence}\label{problems-with-evidence}}
...
\begin{quote}
heart disease \textbf{increases} with alcohol consumption
\end{quote}

but then:

\begin{quote}
heart disease \textbf{decreases} with alcohol consumption
\end{quote}
...
Case A: there are 50 studies which suggest a strong positive link and 50 studies which suggest a strong negative link. We could combine these into a single map (see section xx) claiming a \emph{zero} link but evidence of 100.

Case B: there are 100 studies which \emph{all} suggest a \emph{zero} link. We could combine these into a single map with a combined function claiming a zero link but evidence of 100. It's the same map as Case A! That must be wrong.

\hypertarget{the-chaining-rule-with-loops}{%
\chapter{The chaining rule with loops}\label{the-chaining-rule-with-loops}}

A special case of the mini-map coding rule is when the consequence variable is also one of the influencing variables.

A special case of the join rule is when the resulting mini-map contains a loop or cycle\ldots{}

{[}add something here!{]}

\hypertarget{conceptual-rule}{%
\chapter{The rule for conceptual links}\label{conceptual-rule}}

\begin{quote}
Conceptual links like ``B is part of the definition of E'' can be coded analogously to causal links but should be clearly distinguished from them.
\end{quote}
...
The Soft Arithmetic of causal maps which include conceptual connections too is really neat. So for example if my project increases the number of girls who are vaccinated, and we also know that total child mortality is partially caused by the number of unvaccinated children (showing conceptual connections as dotted arrows):

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-38-1.pdf}

..\ldots{} then the causal influence travels along the lines in the direction of the arrows, oblivious to the fact that some of them are dotted, and we can infer that my project likely helped to suppress child mortality. Nevertheless the dotted lines are not causal connections.

Where necessary, you should specify the function involved -- in this case, the arithmetical sum.
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-39-1.pdf}

\hypertarget{problems}{%
\section{Problems}\label{problems}}

Understanding conceptual links is crucial for coding. e.g.
...
\hypertarget{combining-extra-information-like-the-values-or-levels-of-variables}{%
\chapter{Combining ``extra'' information like the values or levels of variables}\label{combining-extra-information-like-the-values-or-levels-of-variables}}

What if the mini-maps we want to combine agree in their causal information but differ in the extra information like the values of variables:
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-40-1.pdf}
...
In general, the combined map is only true in the \emph{intersection} of the contexts. (So if one map is true for females, and the other is true only for young people, the combined map is true only for girls).

\hypertarget{replacing-contexts-with-variables}{%
\section{Replacing contexts with variables}\label{replacing-contexts-with-variables}}

If one or more variables B, C,..\ldots{} influence a variable E in a context K, we can recast this so that the information about the context is replaced by an additional variable K which governs the influence of B, C etc on E?

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-41-1.pdf}

That would be great, because this would just be another mini-map:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-42-1.pdf}

\ldots{} and we would have reduced the idea of ``context'' to a variable within an expanded mini-map. The difference is that we (typically) have no information about the relationship between B, C when gender is \emph{not} female.
...
\chapter{Maybe the nodes are not variables, maybe they are \ldots{} propositions, events, schemas?}\label{propositions}}

Mostly in this guide I assume that the component parts of mini-maps are \emph{variables}: things which could be different, take different values. On balance, variables are the best choice for this job.
...
The thing about event-based historical scenarios is that the cause does not just create a causal pulse on the next variable but also \emph{co-creates that next variable}.

{[}add more here!{]}

\hypertarget{propositions-1}{%
\section{Propositions}\label{propositions-1}}

Our nodes could be propositions in the sense of propositional logic. One big difference to a variable-based approach concerns negative statements. If we have a Boolean, false/true \emph{variable} like ``Project is launched, false/true'', this suggests that we are supposed to be equally good at recognising each value of the two values of the variable. Whereas \emph{propositions} are arguably asymmetric in terms of evidence: the evidence for NOT-C is just the absence of the evidence for C, which can be problematic if the search is unbounded, because we don't know if really NOT-C or just that the evidence hasn't arrived yet.
...
\hypertarget{schemas-schemata}{%
\section{Schemas / Schemata}\label{schemas-schemata}}

Most human decision-making is ``fast'' Kahneman (2011) and associative -- the kind of thinking which is better understood in terms of \emph{cognitive schemas}. Causal maps with variables or propositions for their nodes are better at dealing with ``slow'', deliberate reasoning. We could in principle use something like causal maps to model the links between \emph{schemata} - for example, an arrow could show that schema E is more likely to be activated when schema B is activated. The trouble is, it is really difficult to accurately elicit this network from respondents.
...
\hypertarget{variable-based-and-propositional-based-approaches-to-causal-diagrams-chalk-and-cheese}{%
\chapter{Variable-based and propositional-based approaches to causal diagrams: chalk and cheese??}\label{variable-based-and-propositional-based-approaches-to-causal-diagrams-chalk-and-cheese}}
...
In the \emph{proposition-based} approach the items are propositions like ``I lost my job'' or ``the weather is getting worse''. The propositions can stand alone but most often we want them linked together, like ``I lost my job --\textgreater{} I sold my cow''.
...
\hypertarget{the-variable-based-approach}{%
\section{The variable-based approach}\label{the-variable-based-approach}}

This approach is what I've already been suggesting for QuIP coding; I've described it in some detail at \url{http://www.pogol.net/_causal_mapping/index.html}. It's just a more qualitative extension of mainstream causal modelling in social science such as we see in structural equation modelling (SEM) or as described by Judea Pearl; and it's close to the sort of causal networks we see in Fuzzy Cognitive Mapping.

In the \emph{variable-based} approach, the items can be e.g.~\emph{level of} unemployment, \emph{level of} retention of cattle. These variables can be binary (yes/no) or continuous but the most important difference is that they are not true or false; they don't make any claims. Specification of the actual / current / predicted value of the variables is an additional, optional step. The variable-based approach is more general; the proposition-based approach is a special case of it. A \emph{proposition} like ``I sold the cow'' can be expressed using the variable-based approach (more long-windedly) as ``The variable \emph{\texttt{did\ I\ sell\ the\ cow,\ yes/no}} takes the value \emph{yes}''; or even as ``There is a variable called \emph{\texttt{did\ I\ sell\ the\ cow,\ yes/no}}; and this variable takes the value \emph{yes}''. But most often, we don't need to bother with this kind of thing because we are encoding statements like ``loss of job --\textgreater{} sale of cow'' and we don't need the information about whether a particular respondent actually did the selling or not.
...
\hypertarget{more-on-the-proposition-based-approach}{%
\section{More on the proposition-based approach}\label{more-on-the-proposition-based-approach}}

Below is an imaginary causal chain derived from some corresponding statement(s) from a single source. It is a good example of a ``mosquito-bite'' or ``propositional'' chain. Each item can be said to \emph{cause} the next one, down the chain, like dominoes falling over. This idea of ``X causes Y'' is strong in this approach, a familiar idea in ordinary language. You can't say ``X causes Y'' so easily with variables. It is transitive: causality flows down the chain, and if X --\textgreater{} Y and Y --\textgreater{} Z, then X --\textgreater{} Z too. It captures the idea that we can sometimes even see causation happening -- the ball hits the window, which smashes. The mosquito bites the person, who feels the jab. The two parts of the causal link are even hard to separate: it is the ball which ``smashes'' but the glass which ``is smashed''; two sides of the same coin. We feel strongly that we don't need any more waffle, that this is just causation happening in front of our eyes. It a strong kind of evidence, it is ``not circumstantial''.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-43-1.pdf}

In the mosquito-bite approach, every item contributes 100\% to the item downstream of it (and to those preceding that item), and every item can be attributed 100\% to the item upstream of it (and to those preceding that item). Is this a simplification? Of course, but \emph{every} model/approach to causation is a simplification.
...
\hypertarget{the-items-are-propositions-not-variables}{%
\subsection{The items are propositions, not variables}\label{the-items-are-propositions-not-variables}}

The items (the boxes or nodes between which arrows are drawn) are \textbf{propositions} and not, for example, variables. The items themselves are specific empirical (but not causal) claims made in and about a specific context (such as ``X happened'' or ``Y has got worse'') and are, broadly speaking, more or less true or false in that context.

The claims have a varying amount of subjectivity, from a fairly subjective ``this has got worse for me'' to a fairly intersubjective ``the rains have got worse''; nevertheless they are grammatically all \emph{claims}, which can in principle be more or less true or false.

This also means that the items have \emph{no gradation}. They are always one thing or the other. There are no possibilities in between.

\hypertarget{the-items-have-been-individually-experienced}{%
\subsection{The items have been individually experienced}\label{the-items-have-been-individually-experienced}}

In general, the propositions are / have been \textbf{personally true for, or at least personally experienced by the source} -- BSDR (2017) doesn't say this explicitly, and focus group data asks for information on ``people like me''. So, seeing that the absence of X leads to the absence of Y does not give me the right to claim that X leads to Y (even if I have seen X leading to Y in relevantly similar cases, but not my own case.)

\hypertarget{the-propositions-have-contrastive-implications}{%
\subsection{The propositions have contrastive implications}\label{the-propositions-have-contrastive-implications}}

I think that this way of thinking does at least concur with the a mainstream view of causation that a causal claim always \emph{implies} a ``counterfactual'' contrast. If someone makes a claim of the form that ``X caused Y'', but then says that actually even if X had not happened in that way, this wouldn't have made any difference to Y, and that Y can happen or not happen irrespective of X", we would conclude that the speaker does not share our understanding of causality, proposition-based or not, and we should \textbf{not} code the initial statement as a causal claim.
...
\hypertarget{two-sidedness-of-propositions}{%
\subsection{Two-sidedness of propositions}\label{two-sidedness-of-propositions}}
...
\hypertarget{the-items-can-express-changes-states-or-events}{%
\subsection{The items can express changes, states or events}\label{the-items-can-express-changes-states-or-events}}

A strength of this kind of proposition-based analysis of causal statements is that it quite naturally captures the kinds of causal explanations which people actually give. One could argue that we shouldn't even be worrying about what format the antecedent causes take (events, states or whatever), because they are just whatever explanations people give. But this would be problematic. The coder needs to understand the items within a claim, rather than just reproducing them verbatim -- first in order to be able to judge whether a different claim by a different source involves \emph{the same} item or not, and second when reflecting on the other statements made by a respondent changes our understanding of the original statement, and third, most importantly, to check that they really mean \emph{causation} here.

QuIP seems here to be \emph{more} restrictive than the proposition-based approach, sometimes suggesting that only \emph{changes} are relevant, though the definitions in the Glossary in BSDR (2017) are not completely consistent. So, a Driver is ``An action or state (X or Z) behind outcomes (Y).'' and Outcomes are ``Changes (positive or negative) reported by respondents''; as ``outcomes can also become drivers of change'', this means that the apparent distinction between ``actions or state'' on the one hand and ``changes'' on the other hand is blurred (because any one item can be both).

Perhaps classic QuIP coding requires only that final (``no-child'') outcomes which have to be expressed as \emph{changes} in the reference period, whereas other items can be expressed in a variety of ways (though still as propositions). But see below.

\hypertarget{changes}{%
\subsubsection{Changes}\label{changes}}

Changes are familiar items in QuIP coding, for example ..\ldots{}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-44-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-45-1.pdf}

Though there is no hard-and-fast distinction, it can make sense to distinguish events from changes. In this case, the event can also be parsed as a change, from lower to higher food taxes. But \ldots{}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-46-1.pdf}

\ldots{} in this case, the triggering event can hardly be described as a \emph{change}. It is a clear example of a one-off event.
...
Putative static causes like ``unemployment'' are things whose status may not be changing, but still pass the contrastive test. (Of course, causal claims about \emph{changes} in an influencing factor like ``\emph{increasing} unemployment'' are also possible.)

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-47-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-48-1.pdf}

\hypertarget{differences-as-a-general-way-of-expressing-events-states-changes}{%
\subsubsection{``Differences'' as a general way of expressing events / states / changes}\label{differences-as-a-general-way-of-expressing-events-states-changes}}

The distinctions between events, states and changes are not very clear and maybe not important. A response would be to replace these ideas with the broader idea of ``Differences'' or ``deltas'' (\url{https://www.linkedin.com/pulse/dear-world-bank-its-differences-changes-we-look-impact-steve-powell/}).
...
\hypertarget{golden-threads}{%
\subsection{Golden threads}\label{golden-threads}}

One big advantage of QuIP-style coding is that it has the concept of an entire causal chain which is not reducible to a bunch of individual links. Indeed, attribution codes are given to whole chains, \emph{not} to individual links.
...
\hypertarget{limitations-to-the-proposition-based-approach}{%
\chapter{Limitations to the proposition-based approach}\label{limitations-to-the-proposition-based-approach}}

\hypertarget{problems-within-the-proposition-based-approach}{%
\section{\texorpdfstring{Problems \emph{within} the proposition-based approach}{Problems within the proposition-based approach}}\label{problems-within-the-proposition-based-approach}}

\hypertarget{problem-with-only-coding-personal-observations-rather-than-personal-causal-knowledge}{%
\subsection{Problem with only coding personal observations rather than personal causal knowledge}\label{problem-with-only-coding-personal-observations-rather-than-personal-causal-knowledge}}

Suppose a new road is built (this is not the intervention), and nearby farmers have worse yields. I think James says we can process the interview testimony of farmers living near the road (whose yields have decreased) as \texttt{new\ road\ in\ neighbourhood} ---\textgreater{} \texttt{yields\ decreased} but not the testimony of more distant farmers (whose yields did not decrease), even though they are in touch with their peers and have personally experienced the ``other half'' of the counterfactual, and can also attest to the above statement. (Yet I think James says it is OK to code this kind of information when the farmers are reporting together in a focus group.)
...
\hypertarget{problems-with-establishing-common-codes-using-difference-of-degree}{%
\subsection{Problems with establishing common codes using difference of degree}\label{problems-with-establishing-common-codes-using-difference-of-degree}}
...
Fuzzy Sets Ragin and Pennings (2005) provide an interesting approach: the items (like ``unemployment'' or ``democracy'') can carry on as monolithic ideals, but respondent \emph{claims} about them can be moderated according to degree of accordance with that ideal, aka degree of membership of that fuzzy set. However I think this ends up being a kind of variable-based system.

\hypertarget{need-for-a-distinction-between-plus-and-minus-links}{%
\subsection{Need for a distinction between ``plus'' and ``minus'' links?}\label{need-for-a-distinction-between-plus-and-minus-links}}

The rationale for including successive antecedents in a causal chain is that each causally explains the item it influences. In the variable-based paradigm, there is no initial room or need for the idea of ``directionality'' of links. Statements like ``I slept badly --\textgreater{} I failed the exam'' and (a different respondent) ``I studied hard --\textgreater{} I passed the exam'' fit well into the propositional approach. (Expressing a negative or ``minus'' link like ``Sleeping badly --\textgreater{}MINUS--\textgreater{} Passing exam'' fits better into a variable-based approach.)

So far, so good. The problem arises when we (understandably) want to be able to code ``failing the exam'' from one causal chain as the same item as ``passing the exam'' from another chain; if we do recode all ``failing'' items as ``passing'', all the old links will have to have reversed directionality.

It's also worth noting that if a revised QuIP is going to allow minus as well as plus links, the simple ways to summarise project impact on p.~4 and 5 of BSDR (2017) have to be adjusted.

On the other hand, we have to be careful. The proposition-based approach is perhaps more sensitive to ordinary language. Using variables, it is tempting to express ``Getting more vitamins --\textgreater{} Feeling more healthy'' equally as ``Getting fewer vitamins --\textgreater{} Feeling less healthy'' or ``Getting more vitamins --\textgreater{} Feeling less ill'' or ``Getting more vitamins -MINUS-\textgreater{} Feeling more ill''. Whereas there are real distinctions; illness is not merely the opposite of health.

\hypertarget{limitations-problems-with-extending-the-proposition-based-approach}{%
\section{\texorpdfstring{Limitations: Problems with \emph{extending} the proposition-based approach}{Limitations: Problems with extending the proposition-based approach}}\label{limitations-problems-with-extending-the-proposition-based-approach}}

\hypertarget{no-way-to-code-multiple-causation}{%
\subsection{No way to code multiple causation}\label{no-way-to-code-multiple-causation}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-49-1.pdf}
...
The logic of ``attribution'' coding is tricky here. On the surface, this is a negative change, a negative outcome, which is also (partially) attributable to the project! This would make the project seem \emph{bad}, whereas in fact the specific contribution of the project is (from a variable-based perspective) positive.
...
\hypertarget{problems-with-expressing-difference-of-degree}{%
\subsection{Problems with expressing difference of degree}\label{problems-with-expressing-difference-of-degree}}
...
\hypertarget{explicit-causal-knowledge}{%
\subsection{Explicit causal knowledge}\label{explicit-causal-knowledge}}

I think it is also important that what is to be coded is an explicitly \emph{causal}claim. It is not enough that the respondent says ``I noted that X changed. I also noticed that Y changed.'' The QuIP protocol is designed to draw on respondents' underlying causal world-view, and not primarily on observation of co-occurrence. The variable-based approach says this explicitly: we want to get at people's theories. I don't know how the propositional approach can distinguish between ``I noted that X changed. I also noticed that Y changed.'' and ``I noted that X changed. I also noticed that Y changed; X caused Y.''

\hypertarget{vulnerability-to-luck-coincidence}{%
\subsection{Vulnerability to luck / coincidence}\label{vulnerability-to-luck-coincidence}}

Like most practical evaluation techniques, QuIP is vulnerable to over-interpretation of happenstance. For example, if a perfectly-conceived and prepared project is denied success by an incredibly unlucky meteorite strike at the last minute, QuIP is unable even to begin an evaluation, because there are, unluckily, no ``positive changes'' to put at the end of causal chains. QuIP is not alone in this: most narrative approaches to evaluation would have the same problem. Whereas a broader variable-based perspective\footnote{One could argue how ``theory-based evaluation'' fits in here} might ask to what extent the project was the right kind of tool to address the problem, regardless of the fact that it was denied success in an unforeseeable way at the last moment.

This is the difference between evaluating overall \emph{success potential} (variable-based; most interesting for scientists and policy-makers) and \emph{actual success / failure} (proposition-based; most interesting for journalists, law courts, and Ambassadors).
...
\hypertarget{unable-to-code-necessarysufficient-conditions}{%
\subsection{Unable to code necessary/sufficient conditions}\label{unable-to-code-necessarysufficient-conditions}}
...
\hypertarget{weaknesses-of-the-variable-based-approach}{%
\section{Weaknesses of the variable-based approach}\label{weaknesses-of-the-variable-based-approach}}

\emph{Events} are easy to capture with a proposition-based approach and harder to capture with a variables-based approach. So the assassination of Arch-Duke Ferdinand was a causal trigger for WWI, but it is hard to conceive of that assassination as a variable, (something that would still be around but not actuated in an alternative history in which he never went to Sarajevo).

\hypertarget{what-is-to-be-done-suggestions-for-quip}{%
\section{What is to be done? Suggestions for QuIP}\label{what-is-to-be-done-suggestions-for-quip}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi})}
\item
  Just stick to propositional coding. It is simple and can code the majority of statements. We can aggregate items and links as now, e.g.~``X was mentioned 20 times'' but go no further in our interpretation of what that means. Accept that we won't be able to directly code multiple causation, necessary conditions, links with different strengths, statements about lack of change, etc. This approach has nevertheless worked well so far. It is natural, relatively easy to apply and explain, produces highly plausible evidence, and has a very natural and persuasive way of presenting the extent to which a project seems to have led to key outcomes, which is what commissioners want most. In particular it produces ``golden threads'': when one respondent explicitly mentions a causal chain from the project to an outcome.
\item
  As James suggests, coding can be done with the propositional approach and then a variable-based theory built up from that. But this does not get round the limitations of the propositional approach when doing basic coding.
\item
  Use a variable-based model, as already discussed, but to make more effort also to preserve the simplicity and plausibility of the classic model. For example, the existing method of attribution coding can put special emphasis on ``gold threads'' when and if they appear; plus, ``gold threads'' can to a large extent be identified automatically.
\end{enumerate}

\hypertarget{clarifying-how-to-ask-quip-questions-in-contexts-when-the-final-outcomes-are-not-necessarily-changes}{%
\chapter{Clarifying how to ask QuIP questions in contexts when the final outcomes are not necessarily ``changes''}\label{clarifying-how-to-ask-quip-questions-in-contexts-when-the-final-outcomes-are-not-necessarily-changes}}

\hypertarget{classic-quip-the-focus-on-changes}{%
\section{Classic QuIP: the focus on changes}\label{classic-quip-the-focus-on-changes}}
...
Here, I'll look at how QuIP doesn't need to rely too heavily on the idea that the items we code have to be \emph{changes}.

\hypertarget{even-final-outcomes-dont-have-to-be-changes}{%
\section{Even final outcomes don't have to be changes}\label{even-final-outcomes-dont-have-to-be-changes}}
...
\hypertarget{one-off-events}{%
\subsection{One-off events}\label{one-off-events}}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-50-1.pdf}

The commissioner of this evaluation might have been involved in, say, providing the preparation of employers and is very interested in improving the retention of first-time workers within the reference period. But the individual students interviewed as part of this evaluation are not, from their own individual perspectives, describing changes of any kind; they are describing an experience which is by definition a one-off. They are describing Differences made, contrasts with \emph{hypothetical alternatives}, not with \emph{prior states} experienced by the respondents themselves. ``I really knew what to expect and the people knew what support to give, much better than what my cousin said it is like where she lives.''
...
\hypertarget{maintaining-the-status-quo}{%
\subsection{Maintaining the status quo}\label{maintaining-the-status-quo}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-51-1.pdf}

\hypertarget{status-quo-as-a-divergent-counterexample}{%
\subsection{Status quo as a divergent counterexample}\label{status-quo-as-a-divergent-counterexample}}

As I understand it, even when a positive change is expected and reported by many respondents, if we insist on only asking for changes (as QuIP suggests), we suppresses the experience of others who did \emph{not} experience a change. As they can only provide causal chains ending in a change, their experience is missed out. To be sure, as QuIP generally only includes respondents who have been exposed to the project, if a subset of respondents do not report a positive change, at least we can count them. But the respondents might have other information to give about other important non-project factors when the status quo was maintained which simply goes missing if we only ask about changes. What if they would have been able to report that they didn't get good crops because of the presence of a pest, but that information never gets written down?
...
\hypertarget{preceding-items-dont-have-to-be-changes-either}{%
\section{Preceding items don't have to be changes either}\label{preceding-items-dont-have-to-be-changes-either}}

As I understand it, there is less insistence in QuIP that the antecedent items which respondents report as \emph{causes} of the final outcomes themselves have to be changes: they might for example be ``states'' or ``events''.

\hypertarget{how-to-focus-on-the-relevant-contrast}{%
\section{How to focus on the relevant contrast}\label{how-to-focus-on-the-relevant-contrast}}

The trouble is that it is not always obvious \emph{what} we are supposed to be contrasting with the present state of things -- isn't this subjective?
...
\hypertarget{focus-on-initial-items-drivers}{%
\section{Focus on initial items (``drivers'')}\label{focus-on-initial-items-drivers}}

I think that classic QuIP gives a special role to the notion of drivers as the beginning of a causal chain; the project of interest is, of course, by definition a driver, but other ``beginnings'' get special status too?
I'm thinking of Fig 5 in BSDR (2017), which shows a table of outcomes attributed to each driver.
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-52-1.pdf}

In this example, unemployment might be a relevant and actually present causal factor even without government ineptitude.
I'd have thought you'd want to see ``unemployment'' in Fig 5 even though it might never appear at the beginning of a chain? In other words, does it matter to QuIP that an item (apart from the project itself) is at the beginning of a causal chain, does that give it a special status?
...
\hypertarget{coding-individual-causal-fragments-using-propositions-revised}{%
\chapter{Coding individual causal fragments using propositions (revised)}\label{coding-individual-causal-fragments-using-propositions-revised}}
...
Here, I will discuss the most basic coding rule for bare-bones propositional causal maps, and discuss minimal \textbf{extensions} to it. My aim is to be able to code statements like this:
...
\hypertarget{basic-coding-rule}{%
\section{Basic coding rule}\label{basic-coding-rule}}
...
A proposition expresses a \emph{Difference}. A good example of a Difference is a change, but other things can be Differences too (states, events, etc). It says that things are one way rather than some other, perhaps expected, contrast.
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-53-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-54-1.pdf}

\hypertarget{every-causal-propositional-claim-is-actually-three-claims}{%
\subsection{Every causal propositional claim is actually three claims}\label{every-causal-propositional-claim-is-actually-three-claims}}
...
\item
  \emph{the first caused the second}
\end{enumerate}
...
\hypertarget{the-items-have-been-witnessed}{%
\subsection{The items have been witnessed}\label{the-items-have-been-witnessed}}

In general, the propositions are / have been \textbf{personally true for, or at least personally experienced by the source} or sources. We treat the source as a \textbf{witness}.

\hypertarget{the-propositions-have-contrastive-implications-1}{%
\subsection{The propositions have contrastive implications}\label{the-propositions-have-contrastive-implications-1}}
...
\hypertarget{can-include-claims-about-absence-of-expected-states}{%
\subsection{Can include claims about absence of expected states}\label{can-include-claims-about-absence-of-expected-states}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-55-1.pdf}

\hypertarget{mini-extension-causal-chains}{%
\section{Mini Extension: causal chains}\label{mini-extension-causal-chains}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-56-1.pdf}
...
\hypertarget{extension-propositional-claims-about-additive-contributions}{%
\section{\texorpdfstring{Extension: propositional claims about \emph{additive contributions}}{Extension: propositional claims about additive contributions}}\label{extension-propositional-claims-about-additive-contributions}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-57-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-58-1.pdf}
...
Here, we \emph{have} explicitly coded the fact that the respondent said that the overall result was negative, by writing ``small decrease..\ldots{}'' on the consequence item, but we didn't need to because anyone could have worked that out, from the fact that roughly speaking big MINUS + small PLUS = small MINUS.
...
\texttt{got\ subsidy-\/-small\ PLUS-\/-\textgreater{}small\ decrease\ in\ available\ income}

\texttt{prices\ increased)-\/-big\ MINUS-\/-\textgreater{}small\ decrease\ in\ available\ income}

\ldots{} and the individual claims wouldn't be false; the first could be read ``got the subsidy, this made a small positive contribution to available income (which experienced a small resultant decrease overall)''. \textbf{You can even do this in Excel}, you don't need to link the two rows, as long as there is a way to express the strengths of the contributions in each row. In the app it is also possible to code both arrows at once, as you would more generally in the variable-based approach.
...
\hypertarget{encoding-incomplete-claims-about-additive-contributions}{%
\subsection{\texorpdfstring{Encoding \emph{incomplete} claims about additive contributions}{Encoding incomplete claims about additive contributions}}\label{encoding-incomplete-claims-about-additive-contributions}}

\begin{quote}
The river pollution levels have risen a lot; one part of the reason is that factory over there.
\end{quote}

Factory B did not cause \emph{much worse} pollution. There is an implicit claim that there are other causes, that the contributions add up in some way, and that the net effect is a large one. So we could code the above like this:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-59-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-60-1.pdf}

There are issues here about scaling -- what counts as large, a lot, etc.

\hypertarget{examples-we-can-code}{%
\section{Examples we can code}\label{examples-we-can-code}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-61-1.pdf}

\begin{quote}
The project gave us two cows, that was really useful, and the other project gave us eight.
\end{quote}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-62-1.pdf}

\begin{quote}
The project gave us two cows, that was really useful, unfortunately one died of disease.
\end{quote}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-63-1.pdf}

{[}More\ldots{}{]}

\hypertarget{examples-we-cannot-code}{%
\section{Examples we cannot code}\label{examples-we-cannot-code}}
...
\hypertarget{combining-causal-fragments-from-different-sources-using-propositions}{%
\chapter{Combining causal fragments from different sources using propositions}\label{combining-causal-fragments-from-different-sources-using-propositions}}

\hypertarget{same-item}{%
\section{Same item}\label{same-item}}
...
\hypertarget{extension-2-subsuming-items-under-more-general-items}{%
\subsection{Extension 2: Subsuming items under more general items}\label{extension-2-subsuming-items-under-more-general-items}}

This is also a part of classic QuIP. It makes it much easier to aggregate fragments, as above, by recoding \emph{different but similar} items into broader categories.
...
\hypertarget{extension-3-recoding-items-from-different-sources-as-gradations-of-more-general-items}{%
\subsection{\texorpdfstring{Extension 3: Recoding items from different sources as \emph{gradations} of more general items}{Extension 3: Recoding items from different sources as gradations of more general items}}\label{extension-3-recoding-items-from-different-sources-as-gradations-of-more-general-items}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-64-1.pdf}

..\ldots{} which can then be combined into one map.
...
The extension I suggest here allows us to explicitly encode the \emph{degree} of a relationship:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-65-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-66-1.pdf}

as this

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-67-1.pdf}

We still have propositions, and causal claims about simple links between them. But in the \emph{aggregated} map we have combined two propositions into one. Creating something like a proto-variable. This trivial-seeming trick allows us to combine maps on propositions which are in a sense common, but differ by degree, without losing the information about the degree.

\hypertarget{extension-3a-allowing-negative-gradations}{%
\subsection{Extension 3a: allowing negative gradations}\label{extension-3a-allowing-negative-gradations}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-68-1.pdf}
...
\hypertarget{what-these-extensions-still-dont-do}{%
\section{What these extensions still don't do}\label{what-these-extensions-still-dont-do}}
...
\hypertarget{cant-distinguish-between-and-and-or}{%
\subsection{Can't distinguish between AND and OR}\label{cant-distinguish-between-and-and-or}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-69-1.pdf}

In the example of the cow and the feedstuff, the underlying causal map is based on a conjunction; E is true if and only if B is true and C is true. There is no way we could deduce this underlying map from this single observation, and there is no way the respondent could either. The respondent is gifting us the implied underlying causal knowledge, just as they did in the simpler version of the coding rules, above. The \emph{two propositions which they report to us} do not include anything about, for example, cases in which there was a cow but no feedstuff; but the implied causal map does.
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-70-1.pdf}

\hypertarget{necessary-sufficient-conditions}{%
\subsection{Necessary / sufficient conditions}\label{necessary-sufficient-conditions}}
...
\tightlist
\item
  a report of a necessary condition being fulfilled says: B happened, E happened, and B is a necessary cause of E, which means that E couldn't have happened without B. Or: if B doesn't happen, E doesn't happen, but \emph{I don't know what happens if B does happen}.
\item
  a report of a sufficient condition being fulfilled says: B happened, E happened, and B is a sufficient cause of E, which means that NOT-E couldn't have happened without NOT-B, Or: if B does happen, E does happen, but \emph{I don't know what happens if B doesn't happen}. So in a sense a sufficient claim, like a necessary claim, is weaker than an ordinary causal claim.
\end{itemize}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-71-1.pdf}

\hypertarget{coding-causal-maps-with-propositions}{%
\chapter{Coding causal maps with propositions}\label{coding-causal-maps-with-propositions}}

Propositional maps differ from variable-based maps in many ways. Propositional maps encode answers to the question: what happened here and what caused it? Variable-based maps encode answers to the question: what causes what around here?

Here, I will discuss the most basic coding rule for bare-bones propositional causal maps, and discuss some \textbf{extensions} to it.
...
\hypertarget{basic-coding-rule-1}{%
\section{Basic coding rule}\label{basic-coding-rule-1}}
...
This also means that, seen like this, the items have \emph{no gradation}. They are always one thing or the other. There are no possibilities in between. So ``I am fairly happy'' is not specifically contrasted with, say, ``I am very happy'', but with any circumstances in which ``I am fairly happy'' is not true.

We have no way to code a proposition on its own; only as part of a causal statement.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-72-1.pdf}

We can be vague about the timing of things. The consequence has to come after the influence, or be concurrent in such a way that effects are still happening after causes.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-73-1.pdf}

\hypertarget{three-claims}{%
\subsection{Three claims}\label{three-claims}}
...
\hypertarget{the-items-have-been-witnessed-1}{%
\subsection{The items have been witnessed}\label{the-items-have-been-witnessed-1}}

In general, the propositions are / have been \textbf{personally true for, or at least personally experienced by the source} or sources. We treat the source as a \textbf{witness}.

\hypertarget{the-propositions-have-contrastive-implications-2}{%
\subsection{The propositions have contrastive implications}\label{the-propositions-have-contrastive-implications-2}}

If someone makes a claim of the form that ``X caused Y'', then this implies that if X had not happened in that way, Y wouldn't have happened, and that if Y hadn't happened, X can't have happened. This is a strong claim: within this narrow context, holding everything else constant, X is necessary and sufficient for Y. Whenever X is true, Y is true, and vice versa.

This is quite another thing from saying that the people making it have to be in possession of corresponding statistical evidence, and is compatible with a proposition-based as well as a variable-based approach. Mosquito-bite claims are based on a more fundamental experience of causation than comparing a state of affairs with a counterfactual. But they do have counterfactual implications.

\hypertarget{can-include-claims-about-absence-of-expected-states-1}{%
\subsection{Can include claims about absence of expected states}\label{can-include-claims-about-absence-of-expected-states-1}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-74-1.pdf}

\hypertarget{mini-extension-causal-chains-1}{%
\section{Mini Extension: causal chains}\label{mini-extension-causal-chains-1}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-75-1.pdf}
...
\hypertarget{mini-extensions-certainty-and-trust}{%
\section{Mini Extensions: certainty and trust}\label{mini-extensions-certainty-and-trust}}
...
\hypertarget{how-does-classic-quip-extend-this-basic-idea}{%
\section{How does classic QuIP extend this basic idea?}\label{how-does-classic-quip-extend-this-basic-idea}}
...
\hypertarget{extension-1-combining-causal-fragments-from-different-sources-same-item}{%
\subsection{Extension 1: Combining causal fragments from different sources: same item}\label{extension-1-combining-causal-fragments-from-different-sources-same-item}}
...
\hypertarget{extension-2-subsuming-items-under-more-general-items-1}{%
\subsection{Extension 2: Subsuming items under more general items}\label{extension-2-subsuming-items-under-more-general-items-1}}
...
\hypertarget{extension-3-recoding-items-from-different-sources-as-gradations-of-more-general-items-1}{%
\subsection{\texorpdfstring{Extension 3: Recoding items from different sources as \emph{gradations} of more general items}{Extension 3: Recoding items from different sources as gradations of more general items}}\label{extension-3-recoding-items-from-different-sources-as-gradations-of-more-general-items-1}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-76-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-77-1.pdf}

We have discussed this before, perhaps using a notation like ``strength=.3'' instead of ``weaker''.

Here we have two separate causal fragments from separate sources which we have combined afterwards using the ``shared consequence'' rule. The important (implied) step was first to recode this

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-78-1.pdf}

as this

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-79-1.pdf}

We still have propositions, and causal claims about simple links between them. But in the \emph{aggregated} map we have combined two propositions into one. Creating something like a proto-variable. This trivial-seeming trick allows us to combine maps on propositions which are in a sense common, but differ by degree, without losing the information about the degree.

\hypertarget{extension-3a-allowing-negative-gradations-1}{%
\subsection{Extension 3a: allowing negative gradations}\label{extension-3a-allowing-negative-gradations-1}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-80-1.pdf}

This extension does however highlight the problem of the meaning of an aggregated item which represents ``the same'' item in several original sources; we don't in general even necessarily know whether to phrase it as ``increased yields'' or ``decreased yields''. We are really only justified in saying ``changed yields''. We should not be trying to make an actual statement about any change in the total yields amongst many farmers. We don't have enough of the right kind of data for that.

\hypertarget{thats-still-not-enough}{%
\section{That's still not enough}\label{thats-still-not-enough}}
...
\hypertarget{no-way-to-encode-explicit-claims-of-multiple-causation}{%
\subsection{No way to encode explicit claims of multiple causation}\label{no-way-to-encode-explicit-claims-of-multiple-causation}}
...
\texttt{subsidy\ -\/-\textgreater{}\ available\ income\ increased}

\texttt{price\ rises\ -\/-\textgreater{}\ available\ income\ decreased}

because we'd be contradicting ourselves about available income.

We can encode it as

\texttt{subsidy\ +\ price\ rises\ -\/-\textgreater{}\ available\ increase\ decreased\ a\ bit}
...
\hypertarget{no-way-to-encode-implicit-claims-of-multiple-causation}{%
\subsection{No way to encode implicit claims of multiple causation}\label{no-way-to-encode-implicit-claims-of-multiple-causation}}

\begin{quote}
The river pollution levels have risen a lot; one part of the reason is that factory over there.
\end{quote}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-81-1.pdf}

The respondent is a witness to both propositions, but the causal claim is of a \emph{contribution to a whole}. Factory B did not cause much worse pollution. There is an implicit claim that there are other causes, that the contributions add up in some way, and that the net effect is a large one.

We could chicken out and just code like this:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-82-1.pdf}

in which the ``pollution worse'' is ambiguous about how much worse. The contribution is \emph{small} while the net effect is \emph{large}; which claim is being made? To understand the consequence item, we have to shift between the two interpretations.

\hypertarget{solution-propositional-claims-about-contributions-and-assume-additive-causation}{%
\section{Solution: propositional claims about contributions, and assume additive causation}\label{solution-propositional-claims-about-contributions-and-assume-additive-causation}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-83-1.pdf}

Here we are encoding the influence of two propositions on a third. All three propositions are simple and true. But where necessary, we explicitly note that the consequence items \textbf{are \emph{contributions} rather than end results}.
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-84-1.pdf}

Here, we \emph{have} explicitly coded the fact that the respondent said that the overall result was negative, but we could have worked that out, from the fact that big MINUS + small PLUS = small MINUS.

You could say there are \textbf{five} propositions here:
...
\hypertarget{what-these-extensions-still-dont-do-1}{%
\section{What these extensions still don't do}\label{what-these-extensions-still-dont-do-1}}
...
\hypertarget{cant-distinguish-between-and-and-or-1}{%
\subsection{Can't distinguish between AND and OR}\label{cant-distinguish-between-and-and-or-1}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-85-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-86-1.pdf}

\hypertarget{necessary-sufficient-conditions-1}{%
\subsection{Necessary / sufficient conditions}\label{necessary-sufficient-conditions-1}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-87-1.pdf}

\hypertarget{types-of-variable}{%
\chapter{Types of variable}\label{types-of-variable}}

Above, I have briefly explained why I have chosen \emph{variables} as the best candidate for the nodes in our causal maps.
...
\item
  ..\ldots{} and various others, see xx.
\end{itemize}
...
\hypertarget{causal-thinking-is-essentially-contrastive-thinking}{%
\chapter{Causal thinking is essentially contrastive thinking}\label{causal-thinking-is-essentially-contrastive-thinking}}

Variables contrast the actual (or imagined) state of things with possible alternatives.

All causal claims intrinsically have contrastive meaning. (I say ``contrastive'' rather than ``counterfactual'' as the latter would more strictly only refer to past things which can no longer be changed). If A claims that contrails causally influence the weather, and in particular that if if there are a lot of contrails, the weather will be worse, but has no opinion at all about what the weather would be like (controlling for any other influences) if there were fewer contrails, in particular doesn't claim it would be any better, then they haven't understood what ``causal influence'' \emph{means}. In other words a useful and comprehensive general framework for causal maps in social science can start by treating the elements as \emph{variables} in a broad (and mathematical rather than statistical) sense, i.e.~as things that can / could be / could have been different.

{[}xx add more{]}

\hypertarget{lo-hi}{%
\chapter{Lo/hi Variables, types of variable, and contrasts}\label{lo-hi}}

My strong suggestion will be that we can make a lot of sense of causal maps if we think of the nodes/factors/vertexes/tags as variables \emph{taking values between 0 and 1} (or maybe -1 and 1). I call variables of this kind ``lo/hi'' variables for the want of a better word.
...
Another advantage of lo/hi numbers is that not only the current \emph{value} of variables but also in some sense the \emph{strength} of arrows can be captured with them (though we might like to encode the strength of arrows in a broader range, from \textbf{-1}, rather than 0, to 1).

We can directly interpret lo/hi information about the effect of a causal package on a consequence variable as an analog to \emph{effect size}: 1 means total control, 0 means none. When we ``zoom out'' to ask about direct effects of a variable or package on variables further downstream of it, this effect size information is particularly interesting. It is the key to answering a core demand on our app and on reasoning with causal maps; to be able to say that, for example, variable B is a much more important influence on outcome O than variable C.
...
The \emph{details} of how this arithmetic might work are up for grabs. But I will try to sketch out some solutions in the next sections.

It is relatively simple to extend this idea to the range -1 to 1, i.e.~including the idea of variables with negative levels.

We already encode \emph{arrows} as having \emph{negative} influences.

\hypertarget{for-each-...-variables}{%
\chapter{``For each ..\ldots{}.'' variables}\label{for-each-...-variables}}

We also need to deal with repetition and multiple cases (e.g.~when we know about the effects of X on \emph{a} child's health, but we have many children in the programme..\ldots{}). In particular:
- when we have more than one level e.g.~multiple children embedded in multiple classrooms
- when we have higher-level effects e.g.~when classroom effects are not just an aggregate of child effects
- time / duration (for each day, for each semester ..\ldots{})

{[}xx add more{]}

\hypertarget{add-influences}{%
\chapter{Coding specific influences -- individually and in combination}\label{add-influences}}

\hypertarget{summary-of-the-argument-so-far}{%
\section{Summary of the argument so far}\label{summary-of-the-argument-so-far}}

I \protect\hyperlink{manifesto}{argued}\footnote{There aren't any references to literature in this summary, but there are some in the preceding sections} that as evaluators and social scientists we are faced with a bag full of different fragments of causal information which most often will mention several different factors or variables. We need to understand how to encode and synthesise such fragments. In particular, we can use causal maps to show how the fragments link together. Causal maps (actually, directed causal networks) are great because they show how our (putative) knowledge of the world is constructed from relatively stable, portable, fallible, snap-together pieces.

I argued that we need \protect\hyperlink{soft}{rules} to translate backwards and forwards between fragments of (written) causal information and causal maps. In the one direction, the question is how to encode causal information in the form of maps; in the other direction, the question is how to interpret those maps.

Most generally, we need \protect\hyperlink{soft}{rules} about how to \emph{reason} with causal maps (and therefore with the sets of causal fragments which they encode) -- how to see if one map, e.g.~a summary map, (or a summary statement) can be deduced from another. I \protect\hyperlink{meaning-is-arithmetic}{argued} that showing how to reason with causal maps is the same thing as explaining the meaning of their constituent parts and conventions. The maps which result when we encode a causal fragment in a causal map, or join up two mini-maps to make a larger one, \emph{mean} something; but what are the translation rules?

I have presented \protect\hyperlink{mini-map-rule}{some of these rules}, part of what I call ``Soft Arithmetic'' - e.g.~the basic rule for coding a single causal fragment, the rules for joining smaller maps together, etc.
...
\hypertarget{encoding-causal-influences-between-lohi-variables}{%
\section{Encoding causal influences between lo/hi variables}\label{encoding-causal-influences-between-lohi-variables}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-88-1.pdf}
...
Also, (a related problem): how to make sense of any kind of \emph{weight} information like strength, trust, confidence, reliability, probability, class membership etc. Obviously these qualifiers do not all mean the same thing, but we will make a start by looking at how to implement even some generic version. I will show a way to \emph{reduce the influence} of information which is deemed to be, for example, less reliable or trustworthy or simply expresses a weaker connection.

\hypertarget{breaking-down-the-problem}{%
\section{Breaking down the problem}\label{breaking-down-the-problem}}
...
Thus armed, we will be able to tackle the thorny problem of how to combine \emph{different} causal packages influencing the \emph{same} variable (see \protect\hyperlink{shared-consequence}{earlier}), making allowance for any weightings; and for the resulting causal maps, we will be able to ``soft-calculate'' the downstream effects of any given setting of the exogenous (``no-parent'') variables. More generally, we will be able to say things like, for example, ``the effect of B on W, way downstream of it, is likely larger than the effect of C on W, especially when Z is hi''.

\hypertarget{some-mono-functions-functions-with-a-single-influence-variable}{%
\section{Some ``mono functions'': functions with a single influence variable}\label{some-mono-functions-functions-with-a-single-influence-variable}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-89-1.pdf}
...
The different coloured lines are generated by different possible \emph{prior} values of Y, i.e.~values which it would have taken if X hadn't been influencing it.
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-90-1.pdf}

The next example is the same idea, but the influence variable forces the consequence variable to take the opposite value.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-91-1.pdf}

The next two examples are much more interesting: continuous versions of necessary and sufficient conditions. The characteristic upper and lower triangles familiar from empirical presentations of these conditions Goertz(xx) are visible - for NECC, where the influence variable is 0 and for SUFF where it is 1. When the strength is less than 1, the influence variable does not completely determine the consequence variable. When it does not, the information about what value the consequence variable should nevertheless take does not come from arbitrary noise but \emph{from the prior value of the consequence variable}. The influence variable serves to restrict the range of operation of this prior information.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-92-1.pdf} \includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-92-2.pdf}
...
\hypertarget{some-multi-functions-functions-for-packages-with-more-than-one-influencing-variable}{%
\section{Some ``multi functions'': functions for packages with more than one influencing variable:}\label{some-multi-functions-functions-for-packages-with-more-than-one-influencing-variable}}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-93-1.pdf}
...
\hypertarget{SOFTADD}{%
\subsection{SOFTADD}\label{SOFTADD}}
...
\hypertarget{package-free-combinations}{%
\section{``Package-free'' combinations}\label{package-free-combinations}}

Now life starts to get interesting. What happens if we combine two separate ``package-free'' pieces of information about the same consequence variable which do \emph{not} come from the same source and where we have no information about how to combine them? This is a really common situation which happens hundreds of times say in a QuIP study.

These could involve the same or different functions.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-94-1.pdf}
...
Finally, it is perfectly possible to have a causal \emph{package} jammed together with another package which consists of a single influence. Here we have information about how to combine C and T but not about how to combine the influence of that package with the influence of R.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-96-1.pdf}
...
The three different vertical sets of facets or sub-graphs show the three values of the \emph{first} influence variable and the three different horizontal sets of facets or sub-graphs show the value of a \emph{second} influence variable.
...
The graphs also show some additional options (\texttt{type}and \texttt{summary\ function}) which I have not yet discussed.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-97-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-98-1.pdf}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-99-1.pdf}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-100-1.pdf}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-101-1.pdf}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-102-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-103-1.pdf}
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-104-1.pdf}

\hypertarget{contradictions}{%
\section{Contradictions?}\label{contradictions}}

In general, it is not coherent for \emph{the same source} to claim different functional influences from the same or different sets of influence variables \emph{onto the same consequence variable}. This might be surprising. But for example let's think what it would mean if someone claimed simultaneously a necessary and a sufficient condition on the same thing.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-105-1.pdf}

..\ldots{} they couldn't, could they? If the beans are sufficient, you don't need to know anything about the training, so it can't be necessary. If the training is necessary, the beans can't be sufficient (because if there was no training, the beans wouldn't be enough).
...
\hypertarget{building-up-more-functions-from-these-building-blocks}{%
\section{Building up more functions from these building blocks}\label{building-up-more-functions-from-these-building-blocks}}

As we piece together fragments of causal information into a larger network, following these causal rules (or getting the app to do that for us), we will see how downstream variables are in principle influenced by the values of the upstream variables. But we can also ``virtually'' piece together more complicated functions, if we need them, by using \protect\hyperlink{conceptual-rule}{conceptual links} rather than causal links. For example, here we define a conceptual or latent variable ``Job suitability'' -- note the dotted lines -- as the minimum of emotional intelligence and work experience; and we show how this inversely affects the level of conflict in the workplace. By ``\protect\hyperlink{zooming-out}{zooming out}'' we can see that J = MINUS(MIN(E,W)); we have constructed a more sophisticated function by piecing together two from our handful of basic functions.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-106-1.pdf}

\hypertarget{coding-influences-within-in-the-app}{%
\section{Coding influences within in the app}\label{coding-influences-within-in-the-app}}

The app already realises the coded networks as actual R objects, within which the variables or nodes have an attribute called \texttt{level} e.g.~0 or 1, but they can take other values too. Also there is an attribute called \texttt{fun} which stores the function which the variable uses to calculate its own level on the basis of the levels of the variables directly upstream of it. (As there might be more than one package of influence variables, the \texttt{fun} attribute is stored with each such package.)
...
\chapter{One function to rule them all?}\label{universal}}
...
\hypertarget{one-universal-type-of-function-for-coding-causal-claims-with-exclusively-lohi-variables.}{%
\section{One universal type of function for coding causal claims with exclusively lo/hi variables.}\label{one-universal-type-of-function-for-coding-causal-claims-with-exclusively-lohi-variables.}}
...
The purest and least sophisticated strategy to encode causal information, ``\textbf{content-neutral coding}'', does no coding of the causal \emph{content} of the link at all. We interpret an arrow as meaning only

\begin{quote}
C has \emph{some} kind of causal influence on E, but we aren't saying, or don't know, what
\end{quote}

The next step would be a strategy like ``\textbf{purely-positive coding}'' in which we do provide some minimal encoding of the content, but always as some kind of ``positive'' influence, like this:
...
By all means we could use ``content-neutral coding'' to encode willy-nilly the example statements just given as arrows from C to E (or whatever). But we wouldn't be able to do much with the resulting network. We would hardly be justified even in \emph{aggregating arrows} (combining several cases in which several sources mentioned the same arrow, for example, (in the resulting network) using the width of the combined arrows to show the number of mentions). That would be to mix apples and pears.

We will have to try be at least a bit more sophisticated in how we encode causal information as arrows in a causal map.

\hypertarget{absence-of-link}{%
\chapter{Coding a claim about the absence of a causal link}\label{absence-of-link}}

Is it really the same as strength=0. I think not.

\hypertarget{strength-importance}{%
\chapter{Strength / importance}\label{strength-importance}}

\hypertarget{including-the-idea-of-strength-in-our-functional-equation}{%
\section{Including the idea of strength in our functional equation}\label{including-the-idea-of-strength-in-our-functional-equation}}
...
``Strength'' uses a generic way of \emph{tempering} or \emph{weakening} causal information, in particular we use it when the respondent themselves says that the connection is weak.
...
\tightlist
\item
  From: ``source P believes / asserts that `A causes B' and we trust source P 100\%'' we \emph{can} (by definition) conclude that A causes B
\item
...
\hypertarget{direct-ways-of-eliciting-how-important-is-one-variables-contribution-to-another.-fiona}{%
\section{Direct ways of eliciting how important is one variable's contribution to another. (Fiona)}\label{direct-ways-of-eliciting-how-important-is-one-variables-contribution-to-another.-fiona}}
...
\chapter{Causal Inference}\label{inference}}
...
\hypertarget{specifying-the-map}{%
\section{Specifying the map}\label{specifying-the-map}}

\hypertarget{exogenous-variables-and-the-grid}{%
\subsection{Exogenous variables and the Grid}\label{exogenous-variables-and-the-grid}}
...
\hypertarget{inference-in-action}{%
\subsection{Inference in action}\label{inference-in-action}}
...
\hypertarget{upstream-bayesian-inference}{%
\chapter{Upstream (Bayesian) inference}\label{upstream-bayesian-inference}}

Is it enough to process information \emph{down} the network? No.~True, cause can by definition only flow down it. But we might have information which implies something about antecedents, which means we have to reason \emph{backwards}. For example, we might know that the student failed to fulfill one of two conditions for entering a college, and we know they entered it, so we know they must have fulfilled the other condition.
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-107-1.pdf}

\hypertarget{contour}{%
\chapter{Contour}\label{contour}}

Influences don't have to be straight. It is possible to select different contours.

\begin{itemize}
\tightlist
\item
  quick-start: \texttt{(contour=quick-start)}
\item
  slow-start: \texttt{(contour=slow-start)}
\item
  S-shaped: \texttt{(contour=S-shaped)}
\item
  U-shaped: \texttt{(contour=U-shaped)}
\item
  threshold: \texttt{(contour=threshold)}
\item
  straight: \texttt{(contour=straight)} (the default. I don't say ``linear'' as that can mean other things too)
\end{itemize}

\hypertarget{flipping}{%
\section{Flipping}\label{flipping}}

Also, each consequence variable can be ``flipped'' in relation to the combination of its incoming variables. All of these combinations work with any number of incoming variables greater than 1. Flipping is the same as adding a ``MINUS'' variable in between.

\hypertarget{valence}{%
\chapter{Valence}\label{valence}}

Hearts and wedges: Marking which variables are deemed to be \emph{important} / \emph{valuable} (``heart'' symbol?); and which can be \emph{intervened} on (``wedge'' symbol?) In theory, this information can be a way to model or understand or predict someone's behaviour (``they're going to want to intervene on X in order to get Y''). For this to work we need to also encode the fact that they believe that X leads to Y: a causal map about somebody' causal map. We can also generalise this to deal with \emph{multiple} actors each of whom can value and intervene upon different (sets of) variables.
...
\hypertarget{valence-and-direction}{%
\chapter{Valence and direction}\label{valence-and-direction}}

Need to discuss the way ordinary language confuses the valence of something (I like it) with the polarity of the influence(s) affecting it.

``Plus'' vs ``minus'' aka ``positive'' vs ``negative'' influences

Also need to discuss the fact that the psychological implications of ``flipping'' a variable are not equivalent to the logical implications.

E.g. coding a variable as ``High levels of heat'' vs ``Low levels of cold'' etc.

See also Mahoney and Goertz (2004)

\hypertarget{effect-two-kinds}{%
\chapter{Effect: two kinds}\label{effect-two-kinds}}

In a stable universe we can calculate the effect of a variable by tweaking it and describing its projection on some variable of interest. So we can see how twiddling the thermostat setting changes the behaviour of the system, dependent also on ambient temperature.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-108-1.pdf}

But we can also describe the effect of \emph{removing the variable from the system}. For example, what happens to the rest of the household when the thermostat is removed?

\hypertarget{value}{%
\chapter{Interventions and differences}\label{value}}
...
Type \texttt{(.9-\/-.3)} after a variable name to designate a variable as \texttt{intervention=.9;\ base=.3}.

Type \texttt{(-\/-.3)} to designate a variable as base=.3.
...
..\ldots{} means that we intervene to set Variable 1 to 100\% rather than 0\%, and that variable 2 just has a base level of 0, we don't intervene on it.
...
It looks so easy, especially when we have a causal map to hand. In fact, as Pearl insists, we \emph{have} to have a causal map to hand when we try to soft-calculate the contribution which C makes to E.
...
There are a lot of extreme difficulties outside the linear, quantitative comfort zone\ldots{}

\hypertarget{why-reporting-correlation-is-not-helpful}{%
\section{Why reporting correlation is not helpful}\label{why-reporting-correlation-is-not-helpful}}
...
\hypertarget{information-about-the-source-of-our-causal-information}{%
\chapter{\texorpdfstring{Information about the \emph{source} of our causal information}{Information about the source of our causal information}}\label{information-about-the-source-of-our-causal-information}}

Above, we looked at how we can encode `C causes E'. Not at how we can encode ``P believes that C causes E''.

\hypertarget{applying-the-trust-attribute-source-by-source}{%
\section{Applying the ``trust'' attribute source-by-source}\label{applying-the-trust-attribute-source-by-source}}
...
\hypertarget{including-information-about-sources}{%
\section{Including information about sources}\label{including-information-about-sources}}
...
see \protect\hyperlink{maps-of-maps}{later}.

\hypertarget{clusters-of-similar-maps}{%
\chapter{Clusters of similar maps}\label{clusters-of-similar-maps}}
...
\begin{quote}
B-\textgreater{}C-\textgreater{}D-\textgreater{}E-\textgreater{}F-\textgreater{}G-\textgreater{}H-\textgreater{}I
\end{quote}
...
\begin{quote}
B-\textgreater{}C-\textgreater{}D-\textgreater{}F-\textgreater{}H-\textgreater{}I
\end{quote}
...
\hypertarget{maps-of-maps}{%
\chapter{Maps of maps}\label{maps-of-maps}}

Very often some actor's own internal causal map is a relevant factor in our causal map of the whole field. For example, in behaviour change interventions.

We need to be able to encode not only the fact that
...
\begin{quote}
stakeholder Q believes that C leads to E
\end{quote}
...
(and/or Q values C, see Powell(xx)).
\end{quote}

\hypertarget{pdfs}{%
\chapter{Probability density functions}\label{pdfs}}
...
Goertz \& Copestake (xx) suggest including ``certainty'' of a link in the coding form itself, \ldots{} but the ``paradox'' above is more about the certainty we have in aggregated links rather than about coding uncertainty directly.
...
\item
  Or, (I think this is better for most cases) there doesn't need to be \emph{any} change to the coding interface; everything just gets coded with a medium amount of uncertainty.
\end{itemize}
...
\hypertarget{part-simplification}{%
\part{Simplification}\label{part-simplification}}

\hypertarget{simplifying-causal-maps-aggregation-and-filtering}{%
\chapter{Simplifying causal maps: aggregation and filtering}\label{simplifying-causal-maps-aggregation-and-filtering}}

\begin{quote}
The best answer to most causal questions about a causal map is another causal map: a simpler one which can be derived from the first.
\end{quote}

So you have coded multiple fragments of causal information. The app \emph{can} put them together into a combined network with no aggregation at all -- one line for each causal link etc -- but usually this will need simplifying.
...
In the QuIP protocol, only one code is made per respondent per domain or question, even if a link is mentioned several times. This can already be considered a kind of summary.

\hypertarget{why-simplify-a-causal-map}{%
\section{Why simplify a causal map?}\label{why-simplify-a-causal-map}}

To count as knowledge, it has to fit in your head.

A presentation of a causal map should fit on a page of A4.

It should be simple enough that the user, with a bit of work, can get a \emph{causal gestalt} -- this might even be likened to muscle memory, a physical feeling for how things fit together, what you have to wiggle to make something else jiggle.

Data-led simplification makes use of different metrics which we can define for the variables, arrows etc in the map. The following sections look at different available metrics.

\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}

How are we going to aggregate a network?

\hypertarget{the-tables-available}{%
\section{The tables available}\label{the-tables-available}}

The app starts its aggregation looking at these three tables:

\hypertarget{table-of-statements}{%
\subsection{Table of Statements}\label{table-of-statements}}

This has not only the texts of the actual statements but also optionally user-defined attributes like source ID, gender, district, question number, whatever columns were in the data which was imported at the beginning of the project. Also, if a column has a special name like \texttt{Age.District.Sex} and contains values like\texttt{22.North.M} it will be split into separate attributes, say, Age = 22, District = North, Sex = M.

\hypertarget{table-of-variables}{%
\subsection{Table of Variables}\label{table-of-variables}}
...
\hypertarget{table-of-arrows}{%
\subsection{Table of Arrows}\label{table-of-arrows}}
...
At the moment, the app now carries out the following steps, in this order. I'd thought that this process would have to be completely user-defined, but in fact at least part of this ordering is inevitable. So the order of the steps is hard-coded but the user can switch the steps on or off, and tweak them as described by fiddling with the settings in the \texttt{Display} tab in the app.

\hypertarget{the-steps-taken-by-the-app-when-simplifying-a-network-according-to-user-commands-andor-automatically-if-requested.}{%
\section{The steps taken by the app when simplifying a network according to user commands and/or automatically if requested.}\label{the-steps-taken-by-the-app-when-simplifying-a-network-according-to-user-commands-andor-automatically-if-requested.}}

\hypertarget{inference-1}{%
\subsection{Inference}\label{inference-1}}

(Not strictly part of the simplification process but mentioned here to keep its place in the chain of calculations.)

\emph{If infer is set to T or TRUE}: Calculate the levels of downstream variables given the levels of upstream variables. This means that for each variable, the app has to know how to combine the upstream information. That is what is stored in the \texttt{fun} (i.e., ``function'')attribute of each variable, e.g.~\texttt{AND} and \texttt{OR}. The \texttt{fun} attribute can be set manually in the Arrows table or in some cases by using the ``Interaction'' widget in the Coding tab.

\hypertarget{calculation-of-source-by-source-data}{%
\subsection{Calculation of source-by-source data}\label{calculation-of-source-by-source-data}}

(Not strictly part of the simplification process but mentioned here to keep its place in the chain of calculations.)

Here we can calculate metrics like ``conspicuous absence'', see xx.

\hypertarget{merging-together-variables-which-are-in-the-same-cluster}{%
\subsection{Merging together variables which are in the same cluster}\label{merging-together-variables-which-are-in-the-same-cluster}}
...
\emph{If \texttt{variablemerge} is set to TRUE}: Any variables which have the same letter or string (``A'', ``4'', ``Cluster1'' or whatever) in the \texttt{Cluster} column are merged into one. Any arrows to the component variables are re-routed to this new cluster variable, TODO: how are variable attributes like \texttt{valence} aggregated?.
...
\hypertarget{merge-information-about-statements-into-the-arrows-which-are-based-on-them}{%
\subsection{Merge information about Statements into the arrows which are based on them}\label{merge-information-about-statements-into-the-arrows-which-are-based-on-them}}
...
\hypertarget{filter-sources}{%
\subsection{Filter sources}\label{filter-sources}}
...
\hypertarget{hard-coded-calculations}{%
\subsection{Hard-coded calculations}\label{hard-coded-calculations}}

Hard-coded calculations for the arrows combining the attributes. At the moment, only \texttt{wtrust} = \texttt{strength} weighted by \texttt{trust}. (TODO: allow user-defined calculations)

\hypertarget{merge-arrows}{%
\subsection{Merge arrows}\label{merge-arrows}}

\emph{If \texttt{arrowmerge} is set to TRUE:} All arrows connecting the same pair of variables are merged into one arrow. The attribute \texttt{frequency} i.e.~count of the number of arrows is added automatically to the new combined arrow. Other attributes (strength, trust, sensitivity, source, label, etc) are aggregated too (at the moment only sum and mean are calculated, but TODO the app should calculate live any aggregations required by the user for the visualisation e.g.~max, min, user-defined functions etc).

\hypertarget{calculate-quip-style-metrics-e.g.-summaries-per-domain.}{%
\subsection{Calculate QuIP-style metrics, e.g.~summaries per domain.}\label{calculate-quip-style-metrics-e.g.-summaries-per-domain.}}

\hypertarget{filter-out-arrows-below-a-minimum-frequency}{%
\subsection{Filter out arrows below a minimum frequency}\label{filter-out-arrows-below-a-minimum-frequency}}

\emph{If \texttt{arrowminimumfrequency}} is larger than 0, any arrows with frequency below this threshold will be excluded from now on.

\hypertarget{add-arrow-attributes-to-variables}{%
\subsection{Add arrow attributes to variables}\label{add-arrow-attributes-to-variables}}

\emph{If \texttt{variablejoinedges} is \texttt{TRUE}}: The attributes of arrows to and from a variable are added to it. So for example the variable now ``knows'' how many women mentioned it as the target of a causal claim. This is an important feature of the app and really useful for the final visualisation. This involves two stages of aggregation: first functions like e.g.~mean and sum are used to aggregate the different arrows coming into (\texttt{to}) or going out from (\texttt{from}) a given variable, and secondly a function is used to aggregate the \texttt{from} and \texttt{to} information. This means that at this stage, a variable can potentially have information like mean\_age\_mean\_mean, i.e., working backwards, take the mean of the arrows merged in step 5 above, then the mean of arrows from several variables linking from or to this one, then the mean of this \texttt{from} and \texttt{to} information. Also available are a host of attributes like from\_age\_mean\_sum and so on. It's quite bewildering!

\hypertarget{filter-out-variables-below-a-minimum-frequency}{%
\subsection{Filter out variables below a minimum frequency}\label{filter-out-variables-below-a-minimum-frequency}}

\emph{If \texttt{variableminimumfrequency}} is larger than 0, any variables with frequency below this threshold will be excluded.

Now the aggregation has been completed; the next step is how to use it in the visualisation.

\hypertarget{aggregating-and-filtering-beliefs}{%
\chapter{Aggregating and filtering beliefs}\label{aggregating-and-filtering-beliefs}}
...
\item
  ...
  \end{itemize}
\item
  How to \emph{summarise} multiple such fragments?
...
\hypertarget{aggregation-and-filtering-based-on-face-value}{%
\chapter{Aggregation and filtering based on face value}\label{aggregation-and-filtering-based-on-face-value}}

You can remove items by hand because they are not relevant to your research interest. For example remove items which are not upstream of important outcomes, or downstream of important inputs like interventions.

Or \ldots{}

\hypertarget{aggregation-and-filtering-based-on-particular-research-questions}{%
\chapter{Aggregation and filtering based on particular research questions}\label{aggregation-and-filtering-based-on-particular-research-questions}}

Often you might want to prepare a different view of your causal map in order to answer a specific research question.

\hypertarget{aggregation-and-filtering-based-on-metrics}{%
...
\section{Network metrics}\label{network-metrics}}

{[}add more{]}

\hypertarget{citation-intensity}{%
\chapter{Citation intensity}\label{citation-intensity}}
...
\hypertarget{conspicuous-absence}{%
\chapter{Conspicuous absence}\label{conspicuous-absence}}
...
The basic idea is to look at the \emph{lack} of edges: node pairs (aka variable pairs) which could have an edge (aka arrow) but don't, or have only a thin edge, i.e.~with very few mentions. The made-up example below is supposed to be built up on the basis of edge information derived from say 15 separate respondents. Is the low number of mentions of the arrow between ``better crops including feedstuffs'' and ``healthier pigs'' (below) understandable because they were anyway rarely mentioned together by the different respondents: maybe respondents tended to talk about the pigs, or the crops, but not both? Or is it \emph{conspicuous by its absence}? We can look at the 15 individual ``mini-maps'' produced by the 15 respondents. Did those two nodes in fact appear together in the many of the respondents' mini-maps but without an edge between them? This is something that we can \emph{not} tell just by looking at the map, but a confusion-matrix-type structural analysis \emph{can} tell us.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-109-1.pdf}

One could build a global score for each edge, to complement the score for the frequency of mentions: so, 50\% = this edge is present in 50\% of the mini-maps in which both nodes were mentioned in some way or other. So if it turned out that ``better crops'' and ``healthier pigs'' appeared together in only two mini-maps, it would get a score of 100\% and we would take it more seriously than if it had a score of say 10\%. (We could also go on to look at all the arrows which are not in the map and talk about \emph{how much} they are lacking - a kind of negative causal link.)

The algorithm calculates this score by calculating the individual respondent-level adjacency matrices for the edges mentioned by each respondent, but scoring for \emph{absence} rather than \emph{presence} of edges. So each adjacency matrix only has a 1 for pairs of nodes which are mentioned by the respondent but for which no edge was mentioned. Then combine them into one large adjacency matrix of ``anti-causality''. The trouble is that I am not sure about the details --- not sure if the score should really be punishing situations when people in group 1 say that X leads to Z, and group 2 say that it leads to Y which is \emph{between} X and Z --- that needs some more thought. In this case, you couldn't really say that arrows from X to Z are conspicuously absent, more that these people are thinking in a more detailed way.)

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-110-1.pdf}
...
\hypertarget{notForwards}{%
\chapter{Not forwards}\label{notForwards}}

This metric merely shows whether an arrow in the causal map does not point forwards, i.e.~it points backwards or to another variable in the same ``column'' as it.

Which specific arrow is identified depends on the specifics of the algorithm which calculates the layout (Sugiyama). But the great thing about the metric is it reveals where there are cycles in the network. If you were to delete these arrows, there would be no cycles.

Often, when laid out sensibly, there are fewer loops than people think.

\hypertarget{homogenity-of-paths}{%
\chapter{Homogenity of paths}\label{homogenity-of-paths}}
...
\hypertarget{part-visualising}{%
\part{Visualising}\label{part-visualising}}

\hypertarget{visualising-and-formatting-causal-maps}{%
\chapter{Visualising and formatting causal maps}\label{visualising-and-formatting-causal-maps}}
...
In the \texttt{Display} tab, the user can set almost any visual feature of the network to reflect almost any attribute of the variables, arrows etc.
...
\tightlist
\item
  Hide all variables with some attribute equal to, or less than a certain number, or containing a certain string \ldots{}.
\item
...
\hypertarget{hard-coded-formatting}{%
\section{Hard-coded formatting}\label{hard-coded-formatting}}
...
\hypertarget{constructing-labels-and-tooltips}{%
\section{Constructing labels and tooltips}\label{constructing-labels-and-tooltips}}
...
\hypertarget{focus}{%
\section{Focus}\label{focus}}

\emph{If \texttt{diagramfocus} is TRUE}: This is mainly for interactive viewing, not for printing / exporting. As the user clicks through the different statements in the corpus, the arrows coded on the basis of this statement are highlighted and the others fade. (There is also a ``play'' button which plays through all the statements like a video.)

\hypertarget{coding-using-the-ui-outdated}{%
\chapter{Coding using the UI (outdated)}\label{coding-using-the-ui-outdated}}

\hypertarget{show-that-the-influence-of-one-variable-on-another-has-a-particular-contour.}{%
\section{Show that the influence of one variable on another has a particular contour.}\label{show-that-the-influence-of-one-variable-on-another-has-a-particular-contour.}}

Click a button to put a symbol on the middle of the arrow.

\hypertarget{show-that-variable-x-is-part-of-the-definition-of-variable-y.}{%
\section{Show that variable X is part of the definition of variable Y.}\label{show-that-variable-x-is-part-of-the-definition-of-variable-y.}}

Select a choice from a drop-down. An arrow is drawn from X to Y but it is dashed.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-111-1.pdf}

\hypertarget{show-that-variables-x-and-y-are-linked-by-definition}{%
\section{Show that variables X and Y are linked by definition}\label{show-that-variables-x-and-y-are-linked-by-definition}}

They overlap in meaning.

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-112-1.pdf}
...
\hypertarget{show-that-a-set-of-influence-variables-influence-the-consequence-variable-collectively.}{%
\section{\texorpdfstring{Show that a set of influence variables influence the consequence variable \emph{collectively}.}{Show that a set of influence variables influence the consequence variable collectively.}}\label{show-that-a-set-of-influence-variables-influence-the-consequence-variable-collectively.}}

Click a button to put them all in the same group. They all get a circle on the middle of the arrow and the desired symbol e.g.~\texttt{\&} on top of it.

\includegraphics{img/interaction.png}

\hypertarget{part-summary}{%
\part{Summary}\label{part-summary}}

\hypertarget{summary-of-the-rules-for-inference-in-causal-maps-aka-soft-arithmetic}{%
\chapter{Summary of the rules for inference in causal maps, aka ``Soft Arithmetic''}\label{summary-of-the-rules-for-inference-in-causal-maps-aka-soft-arithmetic}}

\hypertarget{the-inference-rules-for-causal-maps}{%
\section{The inference rules for causal maps}\label{the-inference-rules-for-causal-maps}}

These are rules for reasoning, drawing inferences, with causal maps and/or
ordinary, causal, narrative sentences. These rules tell us how to go from one
set of maps and/or narratives to another.

\hypertarget{the-mini-map-coding-rule}{%
\section{The mini-map coding rule}\label{the-mini-map-coding-rule}}

\begin{quote}
Information like ``the influence variables B, C and D all have some kind of causal influence on the consequence variable E'' can be coded with a \texttt{mini-map} in which one or more variables (the ``influence variables'') are shown with arrows leading to another (``the consequence variable''). The information and the map are equivalent.
\end{quote}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-113-1.pdf}

I call this a ``coding rule'' but more generally it is the first rule of inference for
causal maps. This first rule tells us how to go from a fragment of causal
narrative to a diagram (coding) and back again (interpretation).

Mini-maps are the atoms of causal maps. One action of coding something produces one mini-map. You can build up any causal network from them. In 90\% of practical applications, a mini-map will just contain a \textbf{single} influence variable. But we don't want to get stuck when we need a \textbf{package} of two or more influence variables, e.g.~when someone says ``Both B and C affect E'' or even ``B and C interact to affect E''.

It's crucial that a mini-map codes information about causality, not co-incidence. So the causal map ``C → E'' should \textbf{not} be interpreted along the pattern of ``if you observe (a high level of) C you are more likely to observe (a high level of) E'', though that may or may not be a corollary of the causal information. The strongest and most correct interpretation is ``if you intervene in the system and manipulate C, which may involve breaking any causal links from other factors to C itself, then this manipulation will produce a corresponding effect in E''.

The mini-map is also equivalent to a functional expression: \[E_{posterior} = f(B, C, D, E_{prior})\]
This just says that the value of E is influenced via B, C, and D according to a function \(f\), which shifts the value of E away from our previous or \emph{prior} best guess about E to a value determined not only by the influence variables but also that prior value.

\hypertarget{focus-on-lohi-variables-and-functions-between-them}{%
\section{Focus on ``lo/hi'' variables and functions between them}\label{focus-on-lohi-variables-and-functions-between-them}}
...
\hypertarget{recording-the-actual-values-of-variables}{%
\section{Recording the actual values of variables}\label{recording-the-actual-values-of-variables}}
...
\hypertarget{the-rule-for-conceptual-links}{%
\section{The rule for conceptual links}\label{the-rule-for-conceptual-links}}

\begin{quote}
Conceptual links like ``B is part of the definition of E'' can be coded analogously to causal links but should be clearly distinguished from them.
\end{quote}

The number of unvaccinated children is a conceptual combination of the number of unvaccinated girls and the number of vaccinated boys, but it isn't caused by them. This is NOT a causal link!

The rules for causal maps which include conceptual connections too is really neat. All you have to do is code (in the app / the coding sheet) the fact that the link from a variable / a package of variables is conceptual. So for example if my project increases the number of girls who are vaccinated, and we also know that total child mortality is partially caused by the number of unvaccinated children (showing conceptual connections as dotted arrows):

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-114-1.pdf}

..\ldots{} then the causal influence travels along the lines in the direction of the arrows, oblivious to the fact that some of them are dotted, and we can infer that my project likely helped to suppress child mortality. Nevertheless the dotted lines are not causal connections.

There are two options for coding a conceptual link: directed, as above, and undirected (when the concepts are not the same but overlap in meaning).

\hypertarget{the-rules-for-coding-different-types-of-influence-single-influence-variable}{%
\section{The rules for coding different types of influence; single influence variable}\label{the-rules-for-coding-different-types-of-influence-single-influence-variable}}
...
\hypertarget{the-rules-for-coding-different-types-of-influence-packages-of-multiple-influence-variables}{%
\section{The rules for coding different types of influence; packages of multiple influence variables}\label{the-rules-for-coding-different-types-of-influence-packages-of-multiple-influence-variables}}

The most obvious thing to do is just generalise the above functions, so for
example:
...
this is equivalent to the frequently mentioned ``necessary AND''. When any of B, C, D \ldots{} are zero, the value is zero. When all of them are 1, the value is just the prior value of the consequence variable, i.e.~the influence variables have no effect.

Similarly for SUFF:
- SUFF: \[E_{posterior} = 1 - (1 - B)* (1 - C)* (1 - D)* (1 - E_{prior})\]
...
There are other, simpler functions for more than one influence variable which do not take into account the prior
value of the consequence variable:
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-115-1.pdf}
...
Any other function which makes sense over lo/hi variables, i.e.~between 0 and 1, can be used, for example measures of central tendency and dispersion, like the \emph{average} of the variables in a package. These can be entered directly into the app / coding sheet.

It is important to note that all these functions are applied to a package (even if the package contains only one variable) at once; they are a property of the whole package, not the individual variables. For this reason it is a bit misleading to display the information on the arrows.

\hypertarget{inus}{%
\section{INUS}\label{inus}}

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-116-1.pdf}

Personally I find INUS a big yawn, and there are different ways to render it. Its
main interest is in trying to emulate the ordinary-language expression ``B is a
cause of E''. But as we think harder about causation, this kind of expression
loses its interest. The best answer to the question ``what causes what here'' is
just a causal map, or a simplified causal map (derived from the original using
the rules of inference for causal maps), perhaps with any pedagogical / visual
aids we can give the reader to understand connections and phenomena which
are not obvious.

The INUS-like configuration below is somewhat equivalent to Mackie's
original. One can argue about whether the ANDs should be NECC or whether
the ORs should be SUFFs. Insofar as there are other real conditions Y
mentioned, there is no need to provide an option to code INUS directly
because the Y (or its constituent parts) need to be coded anyway, and INUS is
by its nature a complex beast with multiple parts.

I guess this is a possibility too:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-117-1.pdf}

It would be possible to offer a specific option to directly code this kind of
combination if that was deemed important.
...
\hypertarget{coding-the-strength-of-the-influence-of-a-package}{%
\section{Coding the strength of the influence of a package}\label{coding-the-strength-of-the-influence-of-a-package}}

We can also code the strength of the influence of a package, also between 0 and 1\footnote{It is also possible to think in terms of a \emph{negative} strength, but as we already have MINUS influences, we don't strictly need this.}

\hypertarget{causal-claim-coding-form}{%
\section{Causal claim coding form}\label{causal-claim-coding-form}}

Pick / select / name the ``\textbf{from}'' variable(s) and the ``\textbf{to}'' variable.
...
\item
  ...
  \tightlist
  \item
    If (rarely) there is one or more arrows with a \textbf{different influence} or \textbf{different strength} within this package, need to click / select / specify this differing influence and/or for this variable(s)
  \end{itemize}
\item
  Add specific / ``broken'' aka cited text
\item
  Source and context (can be automatically coded by the app. Can
  then also be linked back to the source and to characteristics of the
  source)
\item
...
\item
  Possibly other kinds of qualifier e.g.~trust, explicitness, but only
  really useful if we have inference rules to tell us what difference they
  make
\end{itemize}

There is no reason to specifically code a causal chain as this can be constructed
from several atomic pieces of coding. Trying to code multiple links at the same
time would just mean repeating the interface n times. An app should give
feedback and show where each new link fits in the grander scheme, and
anyway records the fact that the pieces all come from the same quote.

There is no reason to specifically code cycles or loops because this can be done
just by coding A = f(A) or A = f(B) and then B = g(A) etc.
...
\tightlist
\item
  what happens when coding creates a cycle. The only situation where
  one might want to code such a cycle directly is in the case of so-
  called ``self-loops'' i.e.~``A influences A'' (or ``A and B influence A'').
  At the moment this is allowed in the app, and the coder can select
  the requisite function (e.g.~MINUS) as required, as usual.

  \begin{itemize}
  \tightlist
  \item
    a coder might code ``C influences D'' and, immediately
    after or later, ``B influences A''. This is also fine and also
    does not need a special interface. However in both these
    cases, causal inference becomes potentially very tricky!
  \end{itemize}
\item
  Variables vs.~events
\item
...
\item
  how to combine ``package-free'' combinations of influences
  mentioned separately by different sources. With OR? With
  SOFTADD?
\item
  how to code information about a zero influence
\item
  what role does the number of mentions of a causal link play when a
  causal map is aggregated / simplified?
\item
  Can't we do it simpler? e.g.~just restricting ourselves to PLUS and
  MINUS influences? Do we really need NECC and SUFF?
\item
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-118-1.pdf}

Yes:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-119-1.pdf}

Yes:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-120-1.pdf}
...
\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-121-1.pdf}

There is a workaround:

\includegraphics{Causal_Mapping_files/figure-latex/unnamed-chunk-122-1.pdf}

\hypertarget{part-the-why-question}{%
\part{The why question}\label{part-the-why-question}}

\hypertarget{the-why-question-as-a-generic-method-in-social-research}{%
\chapter{\texorpdfstring{The ``\emph{why} question'' as a generic method in social research}{The ``why question'' as a generic method in social research}}\label{the-why-question-as-a-generic-method-in-social-research}}
...
\hypertarget{steps-to-asking-the-why-question}{%
\section{Steps to asking ``the why question''}\label{steps-to-asking-the-why-question}}

\hypertarget{formulate}{%
\subsection{Formulate \ldots{}}\label{formulate}}

\ldots{} a single question about what causally influences what. There are various different scenarios.
...
\item
  specifically ask people for how beneficial (or detrimental) the variables are. For example asking at the end, ``\ldots{} and which of these things are most important to you?''
\item
...
\hypertarget{pose-the-question}{%
\subsection{Pose the question}\label{pose-the-question}}

\ldots{} to several sources or respondents. You can always add, for example, official documents or even your own considered opinion to the list of evidence and perhaps assign them a high level of trust. You'll usually ask a couple of additional questions about each source like gender, age, status.

\hypertarget{get-the-answers-into-a-spreadsheet}{%
\subsection{Get the answers into a spreadsheet}\label{get-the-answers-into-a-spreadsheet}}

\ldots{} with a column with the replies and a columns for the additional questions. (If you have allowed people to go back and provide another example, it would be good to have an additional column to record the ID of the source)

The subsequent steps (Code, Simplify / synthesise / Analyse) have already been covered in earlier sections.

\hypertarget{when-would-you-not-want-to-use-this-approach}{%
\section{\texorpdfstring{When would you \emph{not} want to use this approach?}{When would you not want to use this approach?}}\label{when-would-you-not-want-to-use-this-approach}}
...
\hypertarget{part-the-why-question-1}{%
\part{The why question}\label{part-the-why-question-1}}

\hypertarget{the-why-question-as-a-generic-method-in-social-research-1}{%
\chapter{\texorpdfstring{The ``\emph{why} question'' as a generic method in social research}{The ``why question'' as a generic method in social research}}\label{the-why-question-as-a-generic-method-in-social-research-1}}

A \emph{method}: formulating a single question about what influences what, and posing it to many sources or respondents to elicit many fragments of causal maps.

\hypertarget{steps-to-asking-the-why-question-1}{%
\section{Steps to asking ``the why question''}\label{steps-to-asking-the-why-question-1}}

\hypertarget{formulate-1}{%
\subsection{Formulate \ldots{}}\label{formulate-1}}
...
\hypertarget{pose-the-question-1}{%
\subsection{Pose the question}\label{pose-the-question-1}}

\ldots{} to several sources or respondents. You can always add, for example, official documents or even your own considered opinion to the list of evidence and perhaps assign them a high level of trust. You'll usually ask a couple of additional questions about each source like gender, age, status.

\hypertarget{get-the-answers-into-a-spreadsheet-1}{%
\subsection{Get the answers into a spreadsheet}\label{get-the-answers-into-a-spreadsheet-1}}

\ldots{} with a column with the replies and a columns for the additional questions. (If you have allowed people to go back and provide another example, it would be good to have an additional column to record the ID of the source)

\hypertarget{start-analysing}{%
\subsection{Start analysing}\label{start-analysing}}
...
\hypertarget{when-would-you-not-want-to-use-this-approach-1}{%
\section{\texorpdfstring{When would you \emph{not} want to use this approach?}{When would you not want to use this approach?}}\label{when-would-you-not-want-to-use-this-approach-1}}
...
``To reduce the redundant data, semi-structured CCM (Laukkanen, 1994, 1998) uses a different format. The interviewed participants answer a series of questions about what factors they perceive to influence the focal phenomenon (anchor theme) and what consequences follow from it. \ldots{} A typical session can cover two to four anchor themes (and the second
data layers) and takes one to two hours.'' Laukkanen and Eriksson (2013)

\hypertarget{the-why-question-asking-about-changes}{%
\chapter{\texorpdfstring{The ``\emph{why} question'' -- asking about changes}{The ``why question'' -- asking about changes}}\label{the-why-question-asking-about-changes}}

\hypertarget{changes-1}{%
\section{``Changes''}\label{changes-1}}
...
\hypertarget{part-using-the-causal-map-app}{%
\part{Using the causal map app}\label{part-using-the-causal-map-app}}

\hypertarget{using-the-causal-map-app-already-partly-outdated}{%
\chapter{Using the causal map app -- already partly outdated!}\label{using-the-causal-map-app-already-partly-outdated}}

At some point this section will be integrated into the body of the rest of the guide. But there is no point doing that now as everything is in flux.
...
\hypertarget{about-the-tables}{%
\section{About the tables}\label{about-the-tables}}

E.g. the table in the \texttt{Import} tab.
...
\hypertarget{left-hand-tabs-for-producing-and-coding-your-diagram}{%
\section{Left-hand tabs for producing and coding your diagram}\label{left-hand-tabs-for-producing-and-coding-your-diagram}}

\textbf{We'll look at each tab in the interface, starting with those on the left-hand side.}

\hypertarget{import}{%
\subsection{Import}\label{import}}

\includegraphics{_img/1.png}
...
There additional columns for data like gender, age or role of the source. These extra categories make your diagrams much more interesting.
...
You \emph{could} make diagrams without looking at this tab, going straight to the \texttt{code} tab and treating everything as coming from one source. It would be a very clunky way to construct a small network, maybe an interesting way of constructing a larger one.
...
As you switch between statements, the arrows you coded for that statement are highlighted in the diagram.

If you have set the ``overview\_column'' setting to be the name of one of the attribute columns in your table of statements, then a button will appear which will show all the statements from the current source.

\hypertarget{widget-for-adding-arrows}{%
\subsubsection{Widget for adding arrows}\label{widget-for-adding-arrows}}
...
\item
  option to mark this combination of incoming variables as interacting, which means a symbol is placed on the corresponding in the diagram. To do this, when coding the fact that more than one variable controls another, i.e.~you have more than one variable name in the top box, check the ``Arrows interact?'' checkbox. There are also be some pre-set options for the interaction like \&, OR etc.
\end{itemize}

This widget is the heart of the app. It is designed to make coding quick and easy.

In the upper box you type the names of one or more variables at the beginning of the arrow(s) and in the lower box you type the names of one or more variables at the end of the arrow(s). Separate the names with a comma or by pressing your \texttt{tab} key. What's cool is that the names of existing variables are suggested as you type. You can accept the suggestion which has a grey highlight just by pressing \texttt{Enter}.

When you have at least one variable name in both boxes, add the corresponding arrow(s) by clicking ``Add this arrow'', or just by pressing \texttt{Tab} until this button is highlighted and pressing \texttt{Enter}.
...
To decide: at the moment, you can't put \emph{several} variables in the upper box \emph{and} several in the lower box.

\hypertarget{widget-to-view-and-edit-arrows}{%
\subsubsection{Widget to View and edit arrows}\label{widget-to-view-and-edit-arrows}}
...
\item
  the \emph{trust} which you as a coder have in the arrow (you can give different levels of trust to different arrows mentioned the same source)
\item
  the \emph{strength} from -1 to +1 which this statement claims the arrow has
\item
  the \emph{label} which appears by default on the arrow
\end{itemize}

\hypertarget{widget-to-view-and-edit-variables}{%
\subsubsection{Widget to View and edit variables}\label{widget-to-view-and-edit-variables}}
...
\item
  don't touch the \texttt{id} column unless you know what you're doing
\item
  you can add a \texttt{group} which is useful for drill-down scenarios
\end{itemize}

You can add any columns you want. If you add one which means something to visNetwork, like \texttt{color.background}, this will be used by the app. You can add arbitrary attributes like \texttt{mynumber} and summaries for it will be available in the visualisation if you put things like \texttt{sum\_mynumber\_sum\_sum} in the settings.
...
\item
  ...
  \tightlist
  \item
    manually in the previous tab (\texttt{code}) by editing the variables table
  \item
    press the button to auto-generate suggestions (uses the ICLUST procedure under the hood) - you can then check and edit these manually in the \texttt{code} tab.
  \item
    or by using the dedicated \texttt{cluster\ variables} widget:
...
\item
  ...
  \tightlist
  \item
    set globally e.g.~change the border colour of the variables (click the coloured area next to \texttt{color.border})
  \item
    set if\ldots{} something equals, (or is greater than, or contains, etc) \ldots{} something else.

    \begin{itemize}
    \tightlist
    \item
      e.g.~you can set all variables to be \texttt{hidden} if the \% of women mentioning it is less than 50\%
    \item
      e.g.~hide all variables containing the word ``village'' (under \texttt{variable}, click the control next to \texttt{hidden} and change it to \texttt{if...}, and then type \texttt{village}) (FIX: think of a way to have multiple such settings for one attribute, e.g.~red for ``village'' and green for ``town'')
    \end{itemize}
  \item
    ...
    \item
      making the width of an arrow dependent on the number of mentions (under \texttt{arrow}, click the control next to \texttt{width} and change it to \texttt{conditional\ on...}, and then select \texttt{frequency})
    \item
      making the colour of a variable depend on the percentage of sources who were women (assuming you have say ``gender'' or ``female'' as one of your columns in the \texttt{Import} tab, under \texttt{variable}, click the control next to \texttt{colour\ background} and change it to \texttt{conditional\ on...}, and then select \texttt{gender})
    \end{itemize}
  \end{itemize}
\item
  there are lots of other possibilities!
\item
  \texttt{Diagram} settings (bottom section)
...
\hypertarget{right-hand-tabs-for-outputs-viewing-diagram-etc}{%
\section{Right-hand tabs for outputs: viewing diagram etc}\label{right-hand-tabs-for-outputs-viewing-diagram-etc}}

\hypertarget{diagram}{%
\subsection{Diagram}\label{diagram}}

\includegraphics{_img/6.png}

On the right is the diagram. You may need to press \texttt{Update} in the top-left corner to get the latest version.

\hypertarget{ways-to-interact-directly-with-the-diagram}{%
\subsubsection{Ways to interact directly with the diagram}\label{ways-to-interact-directly-with-the-diagram}}

\begin{itemize}
\item
  You can drag and drop. However \textbf{at the moment, your edits will not be saved}.
\item
  Zoom
...
\item
  View tooltips when hovering over variables and arrows.
\item
...
\hypertarget{gallery-library}{%
\subsubsection{Gallery / library}\label{gallery-library}}
...
\hypertarget{using-the-causal-map-app-for-real-time-collaborative-theory-construction}{%
\chapter{Using the causal map app for real-time, collaborative theory construction}\label{using-the-causal-map-app-for-real-time-collaborative-theory-construction}}

\begin{figure}
\centering
\includegraphics{C:/Users/steve/AppData/Roaming/Typora/typora-user-images/1558002973590.png}
\caption{1558002973590}
\end{figure}
...
\hypertarget{case-study-the-strawberry-line}{%
\chapter{Case Study: the Strawberry Line}\label{case-study-the-strawberry-line}}

\begin{figure}
\centering
\includegraphics{assets/1561808777750.png}
\caption{1561808777750}
\end{figure}

\hypertarget{theories-gathered-from-activists}{%
\subsection{Theories gathered from activists}\label{theories-gathered-from-activists}}

\hypertarget{the-question-i-sent}{%
\subsubsection{The question I sent}\label{the-question-i-sent}}

\emph{At last night's meeting, we talked about an easy exercise to help us build a ``theory of change'' for our group. Our ``theory of change'' will be a little story and/or diagram which will say, in a nutshell, what we are trying to do, and why. Which levers are we trying to press by doing the things we do, by e.g.~hosting a petition, giving out stickers, and what is supposed to happen as a consequence?}

\emph{One way to do this would be by all sitting round a flipchart, but another way is to ask each of you what you think, and I'll combine the ideas. We can ask the Council officers too and we can compare their ideas to ours.}

\emph{So all you have to do, if you'd like to participate, is to send me your short answer to the two questions below. I've also provided a jokey ``model answer'', below, but you can write what you want, just 1-8 sentences altogether. There is no right or wrong way to answer and don't worry about speling!}

\emph{Here are the two questions:}
\emph{1) What factors could lead to the successful extension of the SL to Clevedon?}
\emph{What do you think are the factors which will help make it happen (or make parts or sections of it happen)? Are there any factors that make it less likely? Do these helping (and hindering) factors influence one another?}
\emph{2) What will happen because of the extension of the SL to Clevedon, once we have it (or parts of it)? What things will get better, will any things get worse? Do these things influence or reinforce one another?}

\emph{Please send your answers to \href{mailto:steve@pogol.net}{\nolinkurl{steve@pogol.net}} (having to post them here might put some people off). It isn't strictly anonymous though - I won't write your name when I combine the information into a bigger picture, but others might be able to guess who said what.}

\emph{Jokey model answer:}

\emph{I think the main point is to stick up so many posters that they can be seen from space. When we have enough, the aliens are sure to see them. As long as we make our idea clear enough, the aliens will obviously agree with us and either exert mind control over the council or just build the extension from space with lasers. Then everyone will see how easy cycling is and will plead with the aliens to build more and more cycle tracks, and everyone will rush to Clevedon to witness the new cycling paradise, as long as the aliens do not scare them off.}

\hypertarget{stories-gathered-from-the-public}{%
\subsection{Stories gathered from the public}\label{stories-gathered-from-the-public}}

(For another purpose)

{[}add more!{]}

\hypertarget{case-study-global-young-academy.-tracing-the-paths-of-gyas-impact}{%
\chapter{Case study: Global Young Academy. Tracing the paths of GYA's impact}\label{case-study-global-young-academy.-tracing-the-paths-of-gyas-impact}}

The \href{https://globalyoungacademy.net/}{Global Young Academy} supports young scientists around the world to connect with other young scientists, develop their careers and work towards solving global problems with science. We asked over 100 people, who had been supported by GYA or were otherwise involved in the programme, to tell us stories about positive changes which had happened because of GYA activities.\footnote{We also asked about negative changes but this was in a different question.} We used an online survey.
...
We analysed the stories looking for examples of where people had said that \emph{\(B\) leads to \(C\)} - for example, where someone said ``I loved the regional meetings because they helped me widen my professional network''. Then all the \emph{\(B\)}s (like, \emph{Global and regional interaction}) and all the \(C\)s (like \emph{networks, friendships, support}) were grouped into themes, which are the boxes in the diagram below: a Theory of Change for GYA. All the individual stories linking \(B\)s and \(C\)s are synthesised into one story using a pre-defined set of analysis steps.

\begin{figure}
\centering
\includegraphics{assets/1559329785486.png}
\caption{1559329785486}
\end{figure}

\begin{quote}
Figure 1: causal map for the Global Young Academy. The numbers in the boxes (and the size of the boxes) reflect the number of people who mentioned a specific factor, and the width of the arrows reflects the number of people who mentioned that specific link. Arrows and boxes which were mentioned only by \textbf{\emph{women}} are coloured \textbf{\emph{red}}; those mentioned only by \textbf{\emph{men}} are coloured \textbf{\emph{blue}} (and shades in between red and blue were mentioned by both). Boxes with \textbf{\emph{white}} borders were mentioned only by \textbf{\emph{younger}} people; boxes with black borders were mentioned only by \textbf{\emph{older}} people (aged over 40, including alumni, professors etc.). Grey shades in between were mentioned by both. In the online version of the diagram, if the user hovers over boxes and arrows they are given more information including individual quotes.
\end{quote}
...
\begin{figure}
\centering
\includegraphics{assets/1559329387947.png}
\caption{1559329387947}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{assets/1559226447376.png}
\caption{1559226447376}
\end{figure}
...
\hypertarget{gya-inputs}{%
\subsubsection{GYA INPUTS}\label{gya-inputs}}

By far the most common theme in this group was \textbf{global and regional interaction,} in particular, meetings.
...
\emph{Note, paragraphs formatted like the one above are quotes from actual respondent stories.}
...
\begin{quote}
Being accepted as a member of the GYA gave me immense confidence boost\ldots{}
\end{quote}

Many respondents highlighted \emph{GYA's inclusive, supportive, interdisciplinary values} as an important input in its own right.

\begin{quote}
I always felt like a bit of an oddball scientist because I do not conform to the ``traditional'' scientist model\ldots{}
\end{quote}

\hypertarget{individual-impact}{%
\subsubsection{INDIVIDUAL IMPACT}\label{individual-impact}}

There was a wide variety of themes concerning impact on individuals. The most common was \textbf{Building networks \& friendships, including mutual support}.
...
We have described the second most common theme as \emph{Adopting GYA's values and vision}. Here, a key personal impact is described as understanding and taking on board a specific GYA way of thinking which involves diversity, cross-disciplinary work, and taking new perspectives on familiar issues.
...
A related theme was general \emph{awareness of diversity}.

\begin{quote}
The GYA has profoundly affected how I think about cultural diversity.
\end{quote}

This theme was later combined with another common theme: \emph{new opportunities and career development}.
...
Another important strand, mentioned by women and men, young and old, was \emph{confidence}. Some people said that holding GYA office helped to improve their confidence, and some mentioned that improved confidence was a factor in helping to empower excluded scientists.

\begin{quote}
The GYA has helped me to connect to a deep commitment I have to engage, to serve and to help build a better world.
\end{quote}

\emph{Management, leadership, teamwork skills and soft skills} were frequently mentioned, for example learning about participatory methodologies.
...
\subsubsection{BROADER IMPACT}\label{broader-impact}}

Many stories mentioned broader impacts beyond those on individuals. The most frequently mentioned was \emph{improved learning and teaching / support to the scholarly environment}.
...
There was also frequent mention of concrete impacts such as \emph{Doing collaborative projects internationally, publications and science addressed to global problems}.

\begin{quote}
Another GYA member and I have a joint research grant together \ldots{} In addition, we also had a joint research publication in a very high impact journal
\end{quote}

\hypertarget{links-between-themes}{%
\subsubsection{LINKS BETWEEN THEMES}\label{links-between-themes}}
...
The most frequently mentioned link was \emph{Global and regional interaction → Building networks \ldots{}}:
...
Another common link was \emph{GYA (non-specific) \ldots{} membership} → \emph{Overcoming discrimination \ldots{} Confidence}
...
As well as people telling us about links from GYA inputs to personal factors and then on to impacts, sometimes they told us about links directly from GYA inputs (on the left of the diagram) to broader impacts (on the right of the diagram), such as the link \emph{GYA (non-specific)} → \emph{Scholarly Environment}\ldots{}:

\begin{quote}
I feel that the existence of GYA is very important because it acts as a cohesive network for all other Young Academies
\end{quote}

One particularly interesting set of links was from the theme ``\emph{Adopting GYA Theory of Change''} \emph{.} This was often mentioned as a driver of broader impact.
...
\hypertarget{appendix-previous-work}{%
\chapter{Appendix: previous work}\label{appendix-previous-work}}
...
\tightlist
\item
  \href{http://journals.sfu.ca/jmde/index.php/jmde_1/article/view/507}{The Book of Why: The New Science of Cause and Effect. Pearl,
  Judea, and Dana Mackenzie. 2018 -- Book Review} This is now out at JMDE.
\item
  \href{https://www.elval.org/en/vizualizaciji-teorije-promjene-kako-razlikovati-uzroke-i-definicije/}{Visualizing Theories of Change -- how not to confuse causes and definitions''.} In: eVAL, Online Journal of the Evaluation Society in Bosnia and Herzegovina.
\item
  \href{https://www.researchgate.net/publication/325880687_Theories_of_Change_making_value_explicit}{Value in Theories of Change}. JMDE.
\item
  \href{http://journals.sfu.ca/jmde/index.php/jmde_1/article/view/561}{Additional notes on the definition of ``Theory''}. Journal of MultiDisciplinary Evaluation, 15(32), 37-52
\end{itemize}
...
\hypertarget{poster}{%
\section{Poster}\label{poster}}

\href{https://drive.google.com/file/d/1_mlzD4sF9xCl7PYd0kuyBYKBWHmzFINZ/view?usp=sharing}{Theorymaker poster -- UKES, 2018}
...
\item
  \href{http://pogol.net/public/article-equity.html}{Value for Money} My first take on the VfM approach by Julian King and OPM, from a Theorymaker perspective.
\item
...
\tightlist
\item
  \href{https://stevepowell.shinyapps.io/TMV-Theorymaker-visnetwork/}{The new app (online version, somewhat behind the development version)}
\item
  \href{https://github.com/stevepowell99/causal-mapping}{The app at GitHub}
\item
  \href{http://slides.theorymaker.info}{Slides}
\item
  \href{http:theorymaker.info}{The original Theorymaker app}
\item
  \href{http://help.theorymaker.info}{Help for the original Theorymaker}
\item
  \href{https://stevepowell.shinyapps.io/theorymaker3/}{Theorymaker 3, the experimental version of Theorymaker} -- all the causal maps can be specified with typed text
\item
  \href{https://stevepowell.shinyapps.io/CausalExplorer/}{Causal Explorer}
\end{itemize}
...
\hypertarget{refs}{}
\leavevmode\hypertarget{ref-Befani2017}{}%
Befani, Barbara, and Gavin Stedman-Bryce. 2017. ``Process Tracing and Bayesian Updating for impact evaluation.'' \emph{Evaluation} 23 (1): 42--60. \url{https://doi.org/10.1177/1356389016654584}.

\leavevmode\hypertarget{ref-bryson2004visible}{}%
Bryson, John M, Fran Ackermann, Colin Eden, and Charles B Finn. 2004. \emph{Visible thinking: Unlocking causal mapping for practical business results}. John Wiley \& Sons.

\leavevmode\hypertarget{ref-BSDR}{}%
BSDR. 2017. ``QuIP and the Yin/Yang of Quant and Qual: How to navigate QuIP visualisations,'' 1--10.

\leavevmode\hypertarget{ref-copestake2019attributing}{}%
Copestake, James, M Morsink, and F Remnant. 2019. ``Attributing Development Impact: the Qualitative Impact Protocol case book.'' March 21, Online.

\leavevmode\hypertarget{ref-Dul2016}{}%
Dul, Jan. 2016. ``Necessary Condition Analysis ( NCA ): Logic and Methodology of `` Necessary but Not Sufficient '' Causality'' 19 (1): 10--52. \url{https://doi.org/10.1177/1094428115584005}.

\leavevmode\hypertarget{ref-Gao2010}{}%
Gao, Xinbo, Bing Xiao, Dacheng Tao, and Xuelong Li. 2010. ``A survey of graph edit distance.'' \emph{Pattern Analysis and Applications} 13 (1): 113--29. \url{https://doi.org/10.1007/s10044-008-0141-y}.

\leavevmode\hypertarget{ref-goertz2012tale}{}%
Goertz, Gary, and James Mahoney. 2006. \emph{A tale of two cultures: Qualitative and quantitative research in the social sciences}. Princeton University Press.

\leavevmode\hypertarget{ref-Kahnemann2011}{}%
Kahneman, Daniel. 2011. \emph{Thinking, fast and slow}. New York: Farrar, Straus; Giroux.

\leavevmode\hypertarget{ref-kosko1986fuzzy}{}%
Kosko, Bart. 1986. ``Fuzzy cognitive maps.'' \emph{International Journal of Man-Machine Studies} 24 (1): 65--75.

\leavevmode\hypertarget{ref-koulouriotis2001learning}{}%
Koulouriotis, D E, I E Diakoulakis, and D M Emiris. 2001. ``Learning fuzzy cognitive maps using evolution strategies: a novel schema for modeling and simulating high-level behavior.'' In \emph{Proceedings of the 2001 Congress on Evolutionary Computation (Ieee Cat. No. 01TH8546)}, 1:364--71. IEEE.

\leavevmode\hypertarget{ref-Laukkanen2013}{}%
Laukkanen, Mauri, and Päivi Eriksson. 2013. ``New designs and software for cognitive causal mapping.'' \emph{Qualitative Research in Organizations and Management: An International Journal} 8 (2): 122--47. \url{https://doi.org/10.1108/QROM-08-2011-1003}.

\leavevmode\hypertarget{ref-laukkanen2016comparative}{}%
Laukkanen, Mauri, and Mingde Wang. 2016. \emph{Comparative causal mapping: The CMAP3 method}. Routledge.

\leavevmode\hypertarget{ref-Law2005}{}%
Law, International. 2005. \emph{Process Tracing}. 1997.

\leavevmode\hypertarget{ref-Mahoney2004}{}%
Mahoney, James, and Gary Goertz. 2004. ``The possibility principle: Choosing negative cases in comparative research.'' \emph{American Political Science Review} 98 (4): 653--69. \url{https://doi.org/10.1017/S0003055404041401}.

\leavevmode\hypertarget{ref-markiczy1995method}{}%
Markiczy, Livia, and Jeff Goldberg. 1995. ``A method for eliciting and comparing causal maps.'' \emph{Journal of Management} 21 (2): 305--33.

\leavevmode\hypertarget{ref-Moon2019}{}%
Moon, Katie, Angela M. Guerrero, Vanessa M. Adams, Duan Biggs, Deborah A. Blackman, Luke Craven, Helen Dickinson, and Helen Ross. 2019. ``Mental models for conservation research and practice.'' \emph{Conservation Letters}, no. February: 1--11. \url{https://doi.org/10.1111/conl.12642}.

\leavevmode\hypertarget{ref-Nadkarni2004}{}%
Nadkarni, Sucheta, and Prakash P Shenoy. 2004. ``Nadkarni and Shenoy 2004 -A causal mapping approach.pdf'' 38 (2).

\leavevmode\hypertarget{ref-pearl1986fusion}{}%
Pearl, Judea. 1986. ``Fusion, propagation, and structuring in belief networks.'' \emph{Artificial Intelligence} 29 (3): 241--88.

\leavevmode\hypertarget{ref-pearl2000}{}%
---------. 2000. \emph{Causality: Models, reasoning and inference}. Cambridge Univ Press.

\leavevmode\hypertarget{ref-Ragin2008a}{}%
Ragin, Charles C. 2008. ``Measurement Versus Calibration: A Set-Theoretic Approach.'' \emph{The Oxford Handbook of Political Methodology}, 1--31. \url{https://doi.org/10.1093/oxfordhb/9780199286546.003.0008}.

\leavevmode\hypertarget{ref-Ragin2005}{}%
Ragin, Charles C, and Paul Pennings. 2005. ``Fuzzy Sets and Social Research.'' \emph{Sociological Methods \& Research}. \url{https://doi.org/10.1177/0049124105274499}.

\leavevmode\hypertarget{ref-Bc2006}{}%
Scavarda, A J, T Bouzdine-Chameeva, S M Goldstein, J M Hays, and A V Hill. 2006. ``A methodology for constructing collective causal maps.'' \emph{Decision Sciences} 37 (2): 263--83. \url{https://doi.org/10.1111/j.1540-5915.2006.00124.x}.

\leavevmode\hypertarget{ref-scriven2012logic}{}%
Scriven, Michael. 2012. ``The logic of valuing.'' \emph{New Directions for Evaluation}, no. 133: 17--28. \url{https://doi.org/10.1002/ev}.

\leavevmode\hypertarget{ref-tansey_process_2009}{}%
Tansey, O. 2009. ``Process tracing and elite interviewing: a case for non-probability sampling,'' 481--96.

\leavevmode\hypertarget{ref-trochim2017hindsight}{}%
Trochim, William M. 2017. ``Hindsight is 20/20: Reflections on the evolution of concept mapping.'' \emph{Evaluation and Program Planning} 60: 176--85.

\leavevmode\hypertarget{ref-Zadeh1973}{}%
Zadeh, Lotfi. 1973. ``Outline of a New Approach to the Analysis of Complex Systems and Decision Processes.'' \emph{Systems, Man and Cybernetics, IEEE Transactions on} SMC-3 (1): 28--44. \url{https://doi.org/10.1109/TSMC.1973.5408575}.
